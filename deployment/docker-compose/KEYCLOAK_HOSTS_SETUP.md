...
Both kinotic-org-server (in a container) and your browser (on the host) need to reach
Keycloak at the **same** hostname so the OIDC `iss` claim matches what kinotic-org-server
discovered against, and the browser-facing redirect URL Keycloak generates resolves
correctly:

- **kinotic-org-server → Keycloak** uses `http://keycloak:8888/auth/realms/test`
...
same hostname kinotic-org-server uses internally. The JWT's `iss` claim will then exactly
...
If you get `OIDC issuer validation failed` in the kinotic-org-server logs after a successful
