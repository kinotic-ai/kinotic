...
/**
 * A {@link QueryExecutor} that validates and authorizes the {@link QueryContext}'s entity context before delegating.
...
public class PreAuthorizationExecutor implements QueryExecutor{
    ...
    public <T> Future<List<T>> execute(QueryContext context, Class<T> type) {
        return PersistenceUtil.validateEntityContext(entityDescriptor, context.getEntityContext())
                              .compose(v -> authorizationService.authorize(NamedQueryOperation.EXECUTE, context.getEntityContext()))
    ...
    public <T> Future<Page<T>> executePage(QueryContext context,
                                           ...
                                           Class<T> type) {
        return PersistenceUtil.validateEntityContext(entityDescriptor, context.getEntityContext())
                              .compose(v -> authorizationService.authorize(NamedQueryOperation.EXECUTE_PAGE, context.getEntityContext()))
