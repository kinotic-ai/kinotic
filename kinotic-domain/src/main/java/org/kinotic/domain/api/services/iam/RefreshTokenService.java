 ...
 * each redemption rotates the token and detects reuse of an already-rotated token. A lineage is
 * pinned to the audience it was issued for, so rotation can only ever mint access tokens for that
 * one surface.
 */
