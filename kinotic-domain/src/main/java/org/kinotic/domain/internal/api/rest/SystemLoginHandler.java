...
/**
 * Login route for platform operators (a SYSTEM-scope {@link IamUser} — {@code organizationId}
 * and {@code applicationId} both null). Password-only: system users are provisioned by
 * Kinotic, so there is no signup, SSO, or social path. On success the browser session is
 * established; the STOMP WebSocket handshake then authenticates from that session cookie.
 */
@Component
