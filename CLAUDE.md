...
Participant scope is encoded by subtype, not a flag: `SystemParticipant` (no org), `OrganizationParticipant` (carries `organizationId`), and `ApplicationParticipant` — which **extends** `OrganizationParticipant`, adding `applicationId` and an optional `tenantId`. An APP-scope session intrinsically belongs to an Org, so its `organizationId` is carried on the `ApplicationParticipant` itself (set from the `IamUser` in `DomainUtil.createParticipant`), not derived from the Application at read time. The organization id for an operation therefore comes from the **bound participant** — `securityContext.requireParticipant(OrganizationParticipant.class).getOrganizationId()` resolves for an app user — and should be sourced there, never recovered from a prefix baked into an entity id. A `Participant`'s `tenantId` is distinct from this org/app scope: it identifies which slice of the Application's end-user data the participant belongs to.
