...
Participant scope is encoded by subtype, not a flag: `SystemParticipant` (no org), `OrganizationParticipant` (carries `organizationId`), and `ApplicationParticipant` — which carries its own `organizationId`, `applicationId`, and an optional `tenantId`. `ApplicationParticipant` extends `Participant` directly and is **not** an `OrganizationParticipant`: its `organizationId` identifies the org that **owns** the Application for data ownership and routing, not org-scoped authority. An app participant therefore does **not** satisfy `requireParticipant(OrganizationParticipant.class)` — that call admits org-scope participants only and an APP-scope session is rejected with `AuthorizationException`. This is the boundary that keeps application users out of org-scoped admin services.

An APP-scope session intrinsically belongs to an Org, so its `organizationId` is carried on the `ApplicationParticipant` itself (set from the `IamUser` in `DomainUtil.createParticipant`), not derived from the Application at read time. The organization id for an entity operation therefore comes from the **bound participant** read at its concrete type — e.g. `context.getParticipant().getOrganizationId()` on the `EntityContext`'s `ApplicationParticipant`, which `EntitiesService`/`EntityServiceCache` use to route — and should be sourced there, never via `requireParticipant(OrganizationParticipant.class)` and never recovered from a prefix baked into an entity id. A `Participant`'s `tenantId` is distinct from this org/app scope: it identifies which slice of the Application's end-user data the participant belongs to.
