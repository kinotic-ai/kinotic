...
The node's record keeps a ledger of the room each running workload holds, one entry per workload id, and `freeCpus`, `freeMemoryMb` and `freeDiskMb` are the totals less what the ledger holds. A run holds everything the workload is sized for from the moment it is placed; when it ends — stopped, exited, failed, or reported gone by the node — everything returns to the node and the entry goes. Reserving or releasing the same workload twice changes nothing the second time, so a stop and the node's own report of the same end cannot return the room twice. A node that registers again rebuilds the ledger from the workload records still running on it, so a run the node ended while the server was away is still accounted for.

The node keeps nothing of a run that has ended: once the server holds its outcome, the node removes the VM, its disk and its log files (after the shipper has read them). The workload's record stays on the server with its status and exit code, and its logs stay in the organization's log store under its id. A microservice restarted from the console gets a fresh VM from the project's current deployment.
