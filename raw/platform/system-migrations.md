...
> Writing and maintaining migration scripts for Kinotic's internal system indices.
...
INSERT INTO assets (id, item)
    VALUES ('a1', { kind: 'Book', title: 'Clean Code', isbn: '978-0132350884' }) WITH REFRESH;
```
...
System data that is append-only and time-series — audit trails, events, telemetry — should be backed by an Elasticsearch **data stream** rather than a regular index, so old data ages out and backing indices roll over automatically. Create one from a migration with [`CREATE DATA STREAM`](/apps/reference/migration-sql-grammar#create-data-stream):
...
`DATA_RETENTION` is the native [data stream lifecycle](https://www.elastic.co/docs/manage-data/lifecycle/data-stream) — Elasticsearch deletes data past the period (measured per backing index from rollover, not per document). See the [Migration SQL Grammar](/apps/reference/migration-sql-grammar#create-data-stream) for the full statement and the exact retention semantics.
