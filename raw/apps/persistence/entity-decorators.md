...
Stores a string field as a date, so it can be sorted and range-queried by time. Entities travel between your application and the server as JSON, where a date is an ISO-8601 string, so a date field is declared as a `string` holding a value such as `Date.prototype.toISOString()` returns. A `string[]` field holds a list of dates the same way.

```typescript
import { DateTime } from '@kinotic-ai/persistence'
...
Create a `Date` where you need one with `new Date(entity.createdAt)`. `kinotic generate` rejects a field typed `Date`, since after a read it would hold a string.
...
Marks the time field for stream entities (`EntityType.STREAM`). This field is used as the timestamp for time-series indexing and partitioning. Pair it with `@DateTime` so the field is stored as a date.

```typescript
import { Entity, AutoGeneratedId, TimeReference, DateTime, MultiTenancyType, EntityType } from '@kinotic-ai/persistence'
