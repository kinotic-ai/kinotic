...
Named queries let you define custom query methods on your entity services. You declare methods with the `@Query` decorator using a SQL-like syntax, sync them with the server, and the CLI generates the implementation.
...
1. Run `kinotic sync` to sync your entities and generate the base service classes
2. Add custom methods with the `@Query` decorator to the generated service class
3. Run `kinotic generate` to regenerate the service with your query implementations (`generate` runs locally without syncing to the server)

## Defining Named Queries

Add methods with the `@Query` decorator to your entity service class. Use `:paramName` syntax for parameter binding.
