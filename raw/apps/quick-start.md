...
> Get started building your first Kinotic App -- from installation to a running CRUD service in under five minutes.
...
The `-p` flag publishes the entity definitions so the platform creates the backing storage and registers the services. Sync also generates a complete service class in the `src/generated` directory with methods for CRUD operations and querying.
