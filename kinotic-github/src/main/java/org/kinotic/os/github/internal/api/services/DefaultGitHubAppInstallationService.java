...
/**
 * Default impl: CRUD over the {@code kinotic_github_app_installation} index plus a
 * {@link #findForCurrentOrg()} helper that the SPA polls to render the org-settings
 * "GitHub linked / not linked" card. Inherits org-scope filtering from
 * {@link AbstractCrudService} so callers cannot read installations belonging to other
 * orgs.
 */
@Slf4j
@Component
    ...
    public CompletableFuture<GitHubAppInstallation> findForCurrentOrg() {
        ...
        if (participant == null
            ...
            return CompletableFuture.failedFuture(new AuthorizationException(
                    "ORGANIZATION-scoped session required"));
        }
        String orgId = participant.getAuthScopeId();
