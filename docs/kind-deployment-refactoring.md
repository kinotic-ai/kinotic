...
| `kinotic.local` | Keycloak hostname, OIDC config, CoreDNS, `/etc/hosts` docs | This is the local DNS name used for OIDC flows. Renamed from `structures.local` as part of the rebrand. | Already updated across all config files, Helm values, CoreDNS template, and setup scripts. |
