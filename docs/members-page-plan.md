...
1. **One org-scope user per email, globally — an *org-login* property only.** Sign-up enforces this today via `findFirstOrgUserByEmail`, and the org-login stack *depends* on it: `POST /api/login/lookup` and password auth both resolve the user via the **unscoped** `findByEmail(email)` first-match, and social login via `findAllByOidcIdentity → pickFirst`. Two org-scope rows for one email would make org login nondeterministic (whichever doc ES returns first wins; the other org is unreachable). Therefore **org invites must reject any email that already has an org-scope user anywhere** — not just within the target scope. **Application login is different: it is fully scoped** (`ApplicationLoginHandler` at `/api/app/:orgId/:appId/login/...` uses `findByEmail(email, orgId, appId)`, `authenticateLocal(email, password, orgId, appId)`, and the app's own OIDC callback). So app users are unique *per app* and may legitimately share an email with an org admin or with other apps' users; **app invites use per-app-scope uniqueness, never the global check** (Phase 2 branches on scope; `findFirstOrgUserByEmail` is already org-scope-only, so org invites correctly ignore app users). Multi-org-per-email at the org tier (with a login-time org picker) is real product work and explicitly out of scope.
...
4. **Org scope comes from the bound participant, never the client.** Every `MemberService` call derives `organizationId` from `securityContext.requireParticipant(OrganizationParticipant.class)`. `applicationId` is the only scope value a caller supplies — and when supplied it must be **proven to belong to the participant's org** (load the app, assert its `organizationId` matches) before any scoped operation, or an org admin could manage another org's app. The invite repo inherits the **unscoped** `deleteById`/`findById` from `AbstractRepository`, so `cancelInvite`/enable/disable/remove must **load-and-assert (org, app) match** before mutating — that load-then-check is the security boundary.
...
- An app references its enabled OIDC configs via `Application.oidcConfigurationIds` → `OidcConfigurationService.findEnabledByIds(ids, orgId)` (this is what `ApplicationLoginHandler.handleProviders` uses, returning a **list** — see the `oidcProviderName` rule in Phase 3). `OidcConfigurationService.findOrgLoginConfig(orgId)` returns the org's single SSO config or null. Use these to resolve a scope's OIDC config server-side (org vs app).
- `OidcConfigurationRepository.findById(id, orgId)` is the org-scoped config lookup both gateway OIDC callbacks already use; reuse it for invite-accept.
- There are **two** OIDC callbacks, and invite-accept reuses whichever matches the invite's scope (Phase 4), threading an `inviteToken` through the flow session — no new IdP redirect URI per customer:
  - org SSO → `GET /api/login/callback/sso/:configId` (`OrganizationLoginHandler.completeSsoLogin`), config resolved by `(configId, orgId-from-flow-session)`, creates an `applicationId=null` user;
  - app → `GET /api/app/:orgId/:appId/login/callback/:configId` (`ApplicationLoginHandler.completeAppLogin`), config resolved by `(configId, orgId-from-path)`, creates an `(orgId, appId)` user. `orgId`/`appId` come from the path, so the flow session need only carry `inviteToken` for the app case.
...
4. `internal/api/services/iam/DefaultInviteService.java` (new) — composes `IamUserService.createUser`/`findByEmail`/`findFirstOrgUserByEmail`, `PendingInviteRepository`, `EmailService`, `OrganizationService`. Rules:
   ...
   - `createInvite` normalizes the email, then applies a **scope-branched** uniqueness check (invariant #1) plus a pending-invite check:
     - **org invite** (`applicationId == null`) → reject if `findFirstOrgUserByEmail(email) != null` (global org — message e.g. *"This email already belongs to an organization."*);
     - **app invite** (`applicationId != null`) → reject only if `findByEmail(email, orgId, appId) != null` (in-app; an org user or another app's user with the same email is fine);
     - both → also reject if a pending invite already exists in this scope (`findByEmailAndScope`).
   - Both accept paths funnel through one private `acceptInvite(...)` that calls `createUser` then deletes the invite. **The created user's scope (org vs app) comes entirely from the stored invite's `organizationId`/`applicationId`,** so org and app accepts share the same code unchanged. The OIDC path additionally asserts the invite's `oidcConfigId` matches the callback's config and the IdP `email` claim equals the invited email (case-insensitive); `acceptOidcInvite` derives its scope from the loaded invite (no scope params).
...
3. `internal/api/services/iam/DefaultMemberService.java` (new) — inject `SecurityContext`, domain `IamUserService` + `InviteService`, `OidcConfigurationService`, `ApplicationService`. Private helpers: `requireOrganizationId()` (rejects `ApplicationParticipant` via `requireParticipant(OrganizationParticipant.class)` — the actor is always an **org admin**, who manages both org members and any of the org's app members by passing `applicationId`), `requireOwnedApplication(applicationId)` (when non-null, loads the app and asserts its `organizationId` equals the participant's org — invariant #4 for app scope; called by every scoped method), one `resolveScopeOidcConfig(applicationId)` (used by `inviteOptions` + `inviteMember`), one `loadOwnedMember(id, applicationId)` ((org, app)-match assert + self-lockout guard — an admin can't disable/remove their own account — used by enable/disable + remove). Org id always comes from the participant; `cancelInvite`/`removeMember`/`setMemberEnabled` pass that org to the domain layer so the load-and-assert boundary (invariant #4) holds.
Verify: `:kinotic-os-api:compileJava`. Commit.

### Phase 4 — Gateway invite-accept REST, reusing the existing OIDC callbacks (≤7 files)
Invite-OIDC reuses the **existing** login callbacks rather than adding a new callback route — org invites via `/api/login/callback/sso/:configId`, app invites via `/api/app/:orgId/:appId/login/callback/:configId`. These configs point at customer-managed IdPs; a new `…/api/invite/callback/<id>` redirect URI would have to be registered in every customer's IdP app or invites fail with `redirect_uri_mismatch`. Threading `inviteToken` through the flow session and branching the existing `completeOidcLogin` paths avoids that and keeps to the "one logic path, composed" principle. The accept's `userLookup` *creates* the member (scope taken from the stored invite) instead of looking one up.
...
4. `…/rest/InviteHandler.java` (new) — owns three routes:
   ...
   - `POST /api/invite/start` (OIDC) → load the invite, resolve its scope's config, and `startFlow(ctx, config, <scopeCallbackUrl>, orgId, inviteToken)` so the IdP returns to the matching existing callback (org → `ssoCallbackUrl(configId)`; app → `/api/app/:orgId/:appId/login/callback/:configId`); respond `{redirect}`.
5. `…/rest/OrganizationLoginHandler.java` (edit) — in `completeSsoLogin`, when `result.inviteToken() != null`, run a create-on-accept lookup (`sub -> inviteService.acceptOidcInvite(result.inviteToken(), sub, emailClaim, result.config().getId())`) through the same `completeOidcLogin` path instead of the plain login lookup. Inject `InviteService`. (`acceptOidcInvite` derives the user's scope from the stored invite.)
6. `…/rest/ApplicationLoginHandler.java` (edit) — mirror in `completeAppLogin`: when `result.inviteToken() != null`, the same `acceptOidcInvite(...)` create-on-accept lookup (which yields an `(orgId, appId)` user from the invite). Inject `InviteService`.
7. `…/endpoints/ApiGatewayVertcleFactory.java` (edit) — inject + `mountRoutes` `InviteHandler` next to `organizationSignupHandler`.
...
- **`resendInvite`.**
- **The Application-members page (UI) itself** — the backend (domain, `MemberService`, and both OIDC invite-accept callbacks) is built scope-aware and serves app users now; only the `kinotic-frontend` page that surfaces app-member management is future work. `Members.vue` already takes the `applicationId` prop, so that page is a thin future addition.
