...
| 9 | **`UiDeployment` as a reconciler, and deletion as intent.** `state` with `DeploymentState`; `provision`/`checkProvisioning` become the reconciler with `Requeue.after(30s)`; `schedulePoll`, `staleProvisioning` and `retryProvisioning` collapse into requeue and resync; the UI's finalizer removes its site; `Reconciler<ProjectDeployment>` gains its finalizer, which requests deletion of the microservice and UI deployments, stops the sync and publish workloads, and deletes the machines and its record once the children are gone; `DefaultProjectService.beforeDelete` writes `requestDeletion` in place of deleting. | sized by the reshaping |
