...
```java
// kinotic-domain: org.kinotic.domain.api.model
...
public class Workload implements Watched {
    ...
    private WorkloadStatus status;                      // written only from the node's report, and only forward
...
- `generation`/`observedGeneration` replaces every timestamp comparison on a reconcilable, and a
  ...
  to number, so its reports are ordered by the run's lifecycle (`WorkloadStatus.isAfter`: a run only
  moves forward, so a report of an earlier state is a stale snapshot), never against a
  server-stamped `updated`, which is the comparison the partition trap was made of, and never by a
  node timestamp on the record, which could not order a report against a platform write.
...
public enum WatchEventKind { CONDITION_SET, CONDITION_CLEARED, STATUS_CHANGED, ... }   // each kind arrives with the write that produces it
```
...
- The report guard needs nothing from the wire: the run's lifecycle orders reports, so no
  vm-manager change is needed for it.
...
| # | Phase | Files |
|---|---|---|
| 1 | **Node reports outrank server inference.** (PR #605, reworked.) `Watched`, `WatchedState`, `WatchedParent`, `WatchedType`, `StatusCondition`, `StatusConditionType`, `StatusConditions` (`find`/`has`) in `domain/api/model`; `Workload.state`; `WatchedStateRepository` in kinotic-domain with `setCondition`/`clearCondition` and the Painless functions `state`/`condition`/`reconciled`/`touched` every state script shares, composed by `WorkloadRepository`, which also gains a partial `updateRunSync(id, status, exitCode)` that sets `state.dirty`, so no path saves the whole record; the reaper sets `NODE_UNREACHABLE` and keeps the status and the room; a report from a conditioned workload is applied whatever its timestamp and clears it; an unanswered start or stop marks the workload instead of failing it; deregistering an OFFLINE node records its open runs FAILED; the microservice listing says so; V8 declares `state OBJECT (conditions OBJECT (type KEYWORD, message TEXT, since DATE), parent KEYWORD, dirty BOOLEAN, dirtyAt LONG)` whole. | domain ×7, `Workload`, `WatchedStateRepository`, `WorkloadRepository`, `V8__reconcile_state.sql`, `DefaultVmNodeOrchestrationService`, `DefaultWorkloadOrchestrationService`, the two contracts, `DefaultMicroserviceDeploymentService` + contract, `WorkloadOrchestrationTest` + stub, docs |
| 2 | **The ledger.** `WatchEvent`, `WatchEventKind` (the kinds this phase writes: `CONDITION_SET`, `CONDITION_CLEARED`, `STATUS_CHANGED`; later kinds arrive with their writers), `WatchEventRepository` over `appendToDataStream`; `WatchedStateRepository.record`, called after each of Phase 1's two scripts and by `WorkloadRepository.updateRunSync`, which becomes a script so it returns the record as written; `scriptedUpdateReturningSourceSync` on the template; every writer names its `source`; V8 creates `kinotic_watch_event`. The read side arrives with the console. | domain ×5, the template, `WorkloadRepository`, `WorkloadService` + default + stub, the two orchestration services, V8, tests, docs |
| 3 | **The same contract in the TypeScript packages.** `Watched`, `WatchedState`, `WatchedParent`, `WatchedType`, `StatusCondition`, `StatusConditionType` and `Workload.state` in `@kinotic-ai/management-api`, version bumps and peer floors; the system console renders the condition once the packages are published. | core ×6 + package.json, management-api `Workload.ts` + package.json, peer floors, lock |
| 4 | **`Reconcilable` through `ProjectDeployment`.** `Reconcilable<S>`, `ReconcileState<S>`, `DeploymentState`; `ReconcileStateRepository` with `updateDesired` (noop when equal, creation through `scripted_upsert`) and `reportObserved`; `WatchedDocument` carries the document id and routing a scoped record is stored under, and the template's update variants take them; `ProjectDeployment.state` replaces `status` and `commitSha`, `failureMessage` keeps `DeploymentStatus.message`, and every other write to the record is a named partial update (job run, target, sync machine, artifacts, failure), with every caller (orchestrator, identity and artifact services, portal pages); V8 declares `state` whole; TS model; docs. The read half of the repositories, `findDirty`, `clearDirty`, `findUnreconciled`, `findByParent`, `setParent`, `requestDeletion`, and `beforeDelete` writing intent, arrive with their first consumer in Phase 5. | sized by the reshaping |
| 5 | **The master.** `ReconcileMaster` as an Ignite cluster singleton, `ReconcilerRegistry`, `WatchedRepositories`, `Reconciler`, `Requeue`: the dirty scan with compare-and-clear, keyed queue, backoff, requeue-after, resync over `findUnreconciled`, parent routing, orphan collection; `WatchedStateRepository` gains `findDirty` and `clearDirty`, `ReconcileStateRepository` gains `findUnreconciled`, and `WatchedRepository`/`ReconcilableRepository` in kinotic-domain are what the master reads them through; `findByParent`, `setParent` and `requestDeletion` arrive with the finalizers. `ProjectDeployOrchestrator` becomes `Reconciler<ProjectDeployment>`: its `@Consumer` calls `updateDesired`, the worker runs the job and reports `observed`, a record left `DEPLOYING` by a master that died is run again, a failed build waits for the next push; `deployingProjects`/`pendingDeploys` go. Deletion as intent and the finalizers arrive bottom-up, each with the child that can be asked: the microservice's in Phase 8, the UI's and the project's, with `beforeDelete` writing intent, in Phase 9, so no finalizer is written twice. | master, registry, repositories, `Reconciler`, `Requeue`, orchestrator, its tests |
| 6 | **`VmNode` as a reconcilable, `Workload` as its owned artifact.** Creators set `state.parent`, `WorkloadRepository.scopeOf` is the organization so the parent's repository can find the deployment, and the report path orders a report by the run's lifecycle — `WorkloadStatus.isAfter`, a run only moves forward — in place of the clock guard, so no node timestamp is kept on the record and `reportedAt` is not needed; every write already sets `state.dirty`, so the parent's worker runs on an ended run. `VmNode.state` with `VmNodeState`: `desired` `{ONLINE}` at registration, `observed` `{ONLINE}` or `{DRAINING}` from the heartbeat with `healthMessage` staying on the entity as `failureMessage` does on deployments, `OFFLINE` and `UNREACHABLE` leave `VmNodeStatusType` for the `NODE_UNREACHABLE` condition set by silence or an undeliverable call and cleared by the heartbeat or a registration; `findAvailableNode` selects `state.reconciled = true`; the JDK reaper becomes `Reconciler<VmNode>` on the orchestration service, which marks a silent node and its open runs and requeues itself for when the next heartbeat would be overdue; the master lists every reconcilable once in `init()` through `ReconcilableRepository.findAll`, so that timer chain starts on the node hosting the master; `deregisterNode` refuses a reachable node with open runs, otherwise writes `requestDeletion` (`ReconcileStateRepository`, `DELETION_REQUESTED`), and the finalizer records the open runs FAILED, which routes each to its parent, and deletes the node; registration is a partial upsert of the node's inventory plus the intent, the observation and the cleared condition, and `updateDesired` takes no upsert where a missing record is an error. V8 declares `state` and `healthMessage`; V9 gives the console fixture nodes their state. | sized by the reshaping |
| 7 | **Workload records outlive their runs.** `destroyWorkload` removes the VM, records a run still open `STOPPED` and returns its room, leaving the record; `deleteWorkload` is the one delete: it refuses an open run (`WorkloadStatus.isOpen`, which the node service and `countRunningForNode` share), deletes the run's log streams through `LokiClient.delete` in the organization's tenant, then the record; `TelemetryTenant` in management-api's `api` names the tenant and the workload's stream selector once, for `TenantAccess`, `DefaultLogService` and the orchestrator; `DefaultDeploymentOperationsService.removeMicroservice` destroys and keeps; the retention sweep, `WorkloadRetentionSweeper`, one cluster singleton beside the master deployed by the orchestration service, deletes ended records older than `kinotic.systemApi.workload.retentionDays` through the same call, over `WorkloadService.findEndedBefore`; `values-loki-azure.yaml` gains the compactor retention and `delete_request_store` the delete API needs, as `values-loki.yaml` already has, and the compose Loki the same flags; `LokiClientIntegrationTest` covers the delete; the console destroys an open run and deletes an ended one. | `LokiClient` + default + base client, `TelemetryTenant`, `WorkloadOrchestrationService` + default, `WorkloadService` + default + repository, `WorkloadStatus`, properties, the sweep, helm and compose values, tests, TS proxy, console, docs |
| 8 | **`MicroserviceDeployment` as a reconciler.** `state` with `DeploymentState`, `failureMessage` for `DeploymentStatus.message`, `status` and `commitSha` gone (V8); the deploy job's ensure task writes intent per microservice — `{DEPLOYED, sha}` for the commit's, `{ORPHANED, sha}` for the ones it dropped, the parent set at creation — and waits for the workers to answer, failing on a `FAILED` answer or none in ten minutes; `MicroserviceDeployOrchestrator` is `Reconciler<MicroserviceDeployment>`: a running VM starting the commit's module is kept and adopts the commit, one whose module moved is stopped and replaced, one whose run ended is replaced, after `RESTART_DELAY` when it failed, an open run marked `NODE_UNREACHABLE` counts as possibly running and the condition is set on the deployment beside it, so the worker waits for the node's report or the deregistration and never deploys beside it; a deployment that failed for an intent waits for the next push or a restart; console restart is `stopWorkload` when a VM runs, otherwise `renewDesired` (`ReconcileStateRepository`, `DESIRED_RENEWED`: the generation bumped so the worker answers again), and the ended report routes back to redeploy; `removeMicroservice` writes `requestDeletion` and the finalizer stops its workload, then removes its machine and itself; `issueRuntimeCredentials` records the machine with a partial write; `withRunState` goes, the portal reads `state`. | sized by the reshaping |
| 9 | **`UiDeployment` as a reconciler, and deletion as intent.** `state` with `DeploymentState`, `failureMessage` for what the site last answered, `status` and `commitSha` gone (V8); the deploy job's publish task mints and uploads as before, then writes intent per UI — `{READY, sha}` for the commit's, `{ORPHANED, sha}` for the ones it dropped — and waits for the workers' first answer; `UiDeploymentProvisioner` is one method, `check(deployment, commitSha)`, and `UiDeployOrchestrator` is `Reconciler<UiDeployment>`: `READY` answers the intent, anything else is recorded as `{PROVISIONING, liveCommit}` with the observation and `Requeue.after(30s)`, so `schedulePoll`, `staleProvisioning`, `retryProvisioning`, `checkUiSite` and `provisionUiSite` go, and resync covers a master that restarts; the UI's finalizer runs the removal workload on the project's node, then deletes the record; `Reconciler<ProjectDeployment>` gains its finalizer, which asks for the removal of the microservice and UI deployments, looks again every five seconds until they are gone, then stops the sync and publish workloads, removes the sync machine and deletes its record; `DefaultProjectService.beforeDelete` writes `requestDeletion` in place of deleting, so a management-plane delete never reaches into the system plane. | sized by the reshaping |
| 10 | **`JobRun` liveness.** `JobRun` is `Watched` (V8); `JobRunRepository` is a `WatchedRepository`, its terminal write a scripted `recordOutcome` that touches the state and enters the ledger, so the run's record is created whole at its start and never saved whole again; `JobRunLivenessWatcher` on every node marks a run still `RUNNING` on a node that left `monitorClusterNodes()` with `SERVER_NODE_LEFT`, the second condition type; `JobService.run` takes the `WatchedParent` the run was made by, which `ProjectDeployOrchestrator` sets to its record, so the mark routes to it; its stale-run rule reads the run in place of trusting only its own JVM: a record left `DEPLOYING` is deployed again when the run is gone, ended, or marked. | grind, the orchestrator, TS models |
