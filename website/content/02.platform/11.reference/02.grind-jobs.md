...
Every run persists a `JobRun` — name, version, owner, status, timestamps, error, `resumedFrom`, and the `nodeId` of the executing node — and one `TaskRecord` per task: its `taskPath`, description, status, timestamps, `storeType` and stored name, the serialized `STATE` value, the published `wireValue`, and the failure message when it failed. The records are the run's durable history: what a dashboard renders after the event stream is gone, and what a resume replays from.
