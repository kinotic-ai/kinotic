...
The `wire()` axis answers *who may see it*: a wired value is serialized as JSON onto the run's `TaskCompletedEvent` and persisted on its `TaskRecord`, so watchers — including remote ones, which never receive live objects — can read it. Durable state stays private unless it opts in, a result store can publish without being durable, and `Store.none().wire()` publishes a value that exists only for observers. Wire values only have to serialize, never deserialize, so the `STATE` restrictions on generics do not apply to them.
...
Structure can be decided at runtime: a task may return another `Task`, or a whole `JobDefinition`, instead of a value. The returned work is discovered, reported to watchers as a `TasksDiscoveredEvent`, and executed in place under the producing task's path. A dynamic `Task` inherits the producing task's `Store`.
...
- **`TasksDiscoveredEvent`** — the task tree, at the start of the run and again as dynamic tasks appear
...
Every run persists a `JobRun` — name, version, owner, status, timestamps, error, and `resumedFrom` — and one `TaskRecord` per task: its `taskPath`, description, status, timestamps, `storeType` and stored name, the serialized `STATE` value, the published `wireValue`, and the failure message when it failed. The records are the run's durable history: what a dashboard renders after the event stream is gone, and what a resume replays from.

## Watching a run remotely

`JobService.watchRun(jobRunId)` attaches to a run executing in this process, replaying it from the start; it returns an empty stream for a run that has finished or never started, and subscribing never starts one. `JobMonitoringService` publishes that stream, and the records behind it, to remote callers in the management-api zone:
