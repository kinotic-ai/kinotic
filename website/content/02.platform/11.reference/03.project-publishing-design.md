...
only reads that outcome, and fails naming the state when the storage is not `READY`; nothing
is provisioned by a deployment. A mock provisioner under
...
- **Organization storage.** `AzureOrganizationStorageProvisioner` creates the account, the
  ...
  publishes a UI reads the outcome and fails when the storage is not ready.
