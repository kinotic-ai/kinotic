...
| 5 | Publish UIs | reads the organization's storage (provisioned with the organization; provisioned here only when missing or failed) → container SAS → publish VM uploads → finalize rows; vanished UIs marked `ORPHANED`. `Store.state(UI_DEPLOYMENTS).wire()` |
...
message). Provisioning starts when the organization is created: `DefaultOrganizationService`
calls every `OrganizationProvisioner` (a domain hook) with the saved record, and the
management module's implementation provisions the storage and then the organization's Front
Door resources in the background, recording the outcome on the organization. A deployment
reads that outcome; only storage that is missing or failed is provisioned by a deployment,
before it publishes. A mock provisioner under
...
with the organization once its storage is ready, and again by any site provision that finds
them missing: origin group
...
lasts ten years; each rule is written once, when it is missing, so a rotated account key is
followed by deleting the organization's rule set, which the next site provision of the
organization creates again. Front Door writes are slow, serialized per profile, and answer 409 when one is in
...
- **Organization storage.** `AzureOrganizationStorageProvisioner` creates the account, the
  ...
  `Organization` with a status of `PROVISIONING`, `READY` or `FAILED`. It runs from
  `DefaultOrganizationProvisioner`, the management module's `OrganizationProvisioner`, when
  the organization is created, followed by the Front Door preparation; a deployment that
  publishes a UI reads the outcome, waits for provisioning in flight, and provisions again
  only what is missing or failed. `MockOrganizationStorageProvisioner` points every
