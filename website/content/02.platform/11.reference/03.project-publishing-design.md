...
commit other than the current one and the one it replaced, so the hashed files a tab on the
previous commit still loads stay.
...
The sync step sets, per UI build: `KINOTIC_UI_COMMIT=<sha>` and `KINOTIC_UI_SERVER_URL` (from `DomainProperties.resolveApiBaseUrl()`, placed on the sync
...
- **UIs built in the sync VM.** After the entity sync, `sync.ts` runs `bun run build` in
  every UI artifact with `KINOTIC_UI_COMMIT` and `KINOTIC_UI_SERVER_URL`, the last placed
  on the sync workload from
...
- **The publish task.** The deploy job's fifth task, **Publish UIs**, runs
  ...
  sites, adopts returning ones, orphans vanished ones, keeps the files of the current and
  previous commit and deletes the rest. The exit check is shared with the sync task, and the
