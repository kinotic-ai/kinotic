...
runs it again on request. The management module's provisioner starts the job and returns; the
tasks record their outcome on the organization and the run shows in the system console's job
runs and on the organization's overview, with a **Provision again** action. A deployment only
...
- **Organization storage.** `AzureOrganizationStorageProvisioner` creates the account, the
  ...
  of the `provision-organization-<id>` job that `DefaultOrganizationProvisioner`, the
  management module's `OrganizationProvisioner`, runs when the organization is created and
  whenever the system console's **Provision again** asks; the Front Door preparation is the
  second. A deployment that publishes a UI reads the outcome and fails when the storage is
  not ready.
