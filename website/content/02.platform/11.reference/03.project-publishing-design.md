...
lasts ten years; every provision for the organization writes both rules again with a fresh
one, so re-provisioning any site of the organization is also how a rotated key reaches the
rule set. Front Door writes are slow, serialized per profile, and answer 409 when one is in
...
- **One VM per microservice.** `MicroserviceDeployment` rows, keyed `<projectId>:<name>` so
  ...
  identity, entry and commit. The deploy job's fourth task, **Ensure runtime workloads**,
  keeps running VMs, replaces ended ones and ones whose entry moved, creates missing ones,
...
- **Microservices in the console.** `MicroserviceDeploymentService` (`findAllForProject`,
  `restart`, `remove`), published in the management-api zone from system-api, and the
  portal's deployment page: a microservices table with logs, restart and remove, and the
...
- **Organization storage.** `AzureOrganizationStorageProvisioner` creates the account, the
  ...
  `Organization` with a status of `PROVISIONING`, `READY` or `FAILED`; a second deployment of
  the organization waits for the one in flight, and a failed one is retried by the next.
  `MockOrganizationStorageProvisioner` points every organization at Azurite. The deploy job's
  fourth task, **Ensure organization storage**, runs it for commits that contain a UI.
  Terraform owns the resource group, private-endpoints subnet, private DNS zone and the
...
- **UI deployments and the storage data plane.** `UiDeployment` rows keyed by the site's
  ...
  ready at once.
- **The publish task.** The deploy job's sixth task, **Publish UIs**, runs
...
- **Sites on Front Door.** `FrontDoorUiDeploymentProvisioner` creates, per organization on
  ...
  the route with the UI's prefix as origin path. Every step is get-or-create, a lapsed
  validation gets a new token, profile writes are queued and retried on 409, and a
  provisioning site is polled in the background until it is `READY` or `FAILED`. `remove`
  deletes the route, the domain and the two records. `AzureErrors` classifies the management
...
- **UI deployments in the console.** `UiDeploymentService` (`findAllForProject`,
  `retryProvisioning`, `remove`), published from management-api: listing checks any site
  left provisioning for ten minutes or more, so one whose polling died with the server still
  advances; retry runs the provisioner again and records the outcome; remove takes the site
  down, deletes the UI's prefix in storage and deletes the row. Deleting a project removes
