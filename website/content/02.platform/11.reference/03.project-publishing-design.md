...
- **Organization storage.** `AzureOrganizationStorageProvisioner` creates the account, the
  ...
  kinotic-server roles. The storage firewall admits selected networks only: whether Front
  Door can reach the account without a private link origin is the Phase 7 pre-check.
