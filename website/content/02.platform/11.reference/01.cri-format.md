...
```
srv://tenant-123@kinotic-app.acme-org.orders-app~OrderService
stream://device-42@kinotic-app.acme-org.orders-app~temperature/sensor-1
```
...
| Zone | Description |
|------|-------------|
| `kinotic-app.<organizationId>.<applicationId>` | One application's services. Only that application (and system participants) can call them; they are hosted by the owning organization's runtime, which authenticates as an organization participant. Applications may nest their own sub-zones, e.g. `kinotic-app.acme-org.orders-app.billing`. |
| `kinotic-app-api` | The platform's data plane for applications, such as entity persistence and named query execution. Hosted in-process by the platform only. |
...
Organization and application ids become zone labels. The `kinotic-app` prefix keeps every application zone clear of the platform's own labels, and organization ids may never begin with `kinotic`. Creating an organization or application whose name slugifies to `system` is rejected.

Which zones a connection may address is determined by the authenticated participant. A participant may also address any sub-zone of a zone it is allowed, so `kinotic-app.acme-org.orders-app.billing` is reachable by whoever may reach `kinotic-app.acme-org.orders-app`. An address without a `~` carries no zone at all and is only reachable by system participants.

| Participant | May send to | May subscribe to |
|-------------|-------------|-------------|
| APPLICATION (org, app) | `kinotic-app-api`, `kinotic-app.<org>.<app>` | reply destinations only |
| ORGANIZATION (org) | `os-api`, `kinotic-app-api`, `kinotic-app.<org>.*` | `kinotic-app.<org>.*` — its applications' zones |
...
stream://kinotic-app.acme-org.orders-app~temperature
```
...
stream://kinotic-app.acme-org.orders-app~temperature/sensor-1
```
...
const cri3 = createCRI('stream', 'tenant-123', 'kinotic-app.acme-org.orders-app~orders')
...
```typescript
const cri = createCRI('srv://tenant-123@kinotic-app.acme-org.orders-app~OrderService/placeOrder#2.0.0')

cri.scheme()        // 'srv'
cri.scope()         // 'tenant-123'
cri.hasScope()      // true
cri.zone()          // 'kinotic-app.acme-org.orders-app'
cri.hasZone()       // true
...
cri.version()       // '2.0.0'
cri.hasVersion()    // true
cri.baseResource()  // 'srv://tenant-123@kinotic-app.acme-org.orders-app~OrderService'
cri.raw()           // 'srv://tenant-123@kinotic-app.acme-org.orders-app~OrderService/placeOrder#2.0.0'
```
...
| `srv://kinotic-app-api~org.kinotic.persistence.api.services.JsonEntitiesRepository/save` | A specific method on a platform service |
| `srv://kinotic-app.acme-org.orders-app~OrderService/create#1.0.0` | A versioned method on an application's own service |
| `srv://kinotic-app.acme-org.orders-app.billing~InvoiceService` | A service in an application-declared sub-zone |
| `srv://system~org.kinotic.orchestrator.api.workload.WorkloadOrchestrationService` | A platform-internal service |
| `stream://kinotic-app.acme-org.orders-app~temperature` | An application's event stream |
