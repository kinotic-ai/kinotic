...
Organization and application ids become zone labels, so neither may take one of the labels above. Creating an organization or application whose name slugifies to `app`, `app-api`, `os-api`, or `system` is rejected.
