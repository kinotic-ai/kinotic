...
The development profile sets `kinotic.systemApi.disableAzureStorage`: it marks every
published UI ready without uploading or serving it, and generates no SBOM. To exercise the
real path — sites published into a sites account and served through Front Door, and SBOMs
kept in an organization storage account — point the server at a subscription of your own.
...
domain on a Let's Encrypt wildcard certificate in a key vault of its own, the organization
storage account each deployment's SBOM is kept in, and a service principal for the server
with the roles it needs: Storage Blob Data Contributor on both accounts, with which it signs
each site's upload and removal URLs and each SBOM's upload and read URLs, and Contributor on
the email service, so it sends mail too. The written file is the git-ignored `local` profile:
it turns Azure storage on and names the domain and the two
accounts. The `environment` is yours alone: a site hostname can be bound to one Front Door
profile in all of Azure, so two developers sharing a sites domain would collide.

The portal reads a project's SBOM straight from the organization storage account, which
answers only the origins in the `portal_origins` variable: `http://localhost:5173` unless
`local.auto.tfvars` lists others, such as a tunnel's.
