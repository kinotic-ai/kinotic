...
11. CompleteOrg.vue immediately starts the GitHub App install:
    githubAppInstallations.startInstall("/applications") returns the GitHub install URL
    and the whole tab is redirected there. GitHub returns to /github/install/callback,
    which runs completeInstall and lands on /applications — project creation requires
    the install, so the new org arrives ready. If the install can't start (e.g. a
