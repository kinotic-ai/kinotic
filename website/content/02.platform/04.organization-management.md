...
There are two entry points, both producing an `Organization` and an admin `IamUser` scoped to it. The SPA's `/signup` and `/login` pages currently offer **GitHub only** (hardcoded buttons — every Kinotic project is backed by a GitHub repository, so the founding user signs up with GitHub and installs the GitHub App right after). The email/password entry point and the dynamic provider row below remain fully mounted on the backend; their previous UI (`Signup.vue`, `Login.vue`) is kept in the tree unrouted until other methods return.
...
Once an org exists, members log in through one of three converging paths. The SPA's login page currently renders only the hardcoded "Sign in with GitHub" button (`GithubLogin.vue`); the email-first and password flows below stay mounted on the backend and return to the UI with the other providers.
...
The login page's GitHub button posts straight to the start route below. (`GET /api/auth/org/login/providers` still lists the unique provider keys from the platform social configs — the invite-hint UI on `MembersPage` consumes it, and it remains the source for a dynamic button row when other providers return to the login page.) Clicking the button:
