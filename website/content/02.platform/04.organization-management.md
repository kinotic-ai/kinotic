...
The SPA never exchanges the password for a token and never sends raw passwords over the WebSocket. Non-UI clients (CLI, automation) do not use this path — they authenticate directly at the upgrade instead: a Node client resolves credentials (a `BasicCredentialsResolver`, or the `KINOTIC_CLIENT_ID` / `KINOTIC_CLIENT_SECRET` environment variables) that send `clientId` / `clientSecret` plus the `organizationId` / `applicationId` scope headers.
