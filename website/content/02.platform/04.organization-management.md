...
| `ParticipantIdentity` | A scoped identity carried structurally by `organizationId` / `applicationId`. One row per (person, scope) | Created during signup, by invitation, or by an admin |
...
|---|---|
| Browser SPA | Session cookie established by the REST login; the default credential resolution supplies no headers |
| Node client, credentials | `clientId`, `clientSecret`, plus `organizationId` / `applicationId` scope headers (via `BasicCredentialsResolver` or the `KINOTIC_CLIENT_*` environment variables) |
| Node client, Bearer token | `Authorization: Bearer <jwt>` (via `BearerCredentialsResolver`); a Kinotic JWT carries the `organizationId` / `applicationId` claims |

The browser SPA never holds a JWT — its login establishes a session cookie and that cookie authenticates the upgrade. The Bearer path is for non-browser clients: the CLI obtains an access token through the OAuth device-code grant at `POST /api/auth/oauth/token`, which mints a Kinotic JWT carrying `sub` / `email` / `organizationId` / `applicationId`. For that path the org server validates the JWT signature against its signing keys and creates the `Session`. Every path admits only the server's own kind of participant: a credential that proves an identity of another scope fails as invalid (see [System Security](/platform/system-security#authentication-methods)). The CLI persists a rotating refresh token to mint fresh access tokens before each connect.
...
|---|---|---|---|
| `github` | No id_token — identity is fetched from the row's `userInfoUri` with the exchanged access token, so issuer/nonce validation doesn't apply | Synthesized from the row's `userEmailsUri` — the primary verified address is preferred, any verified address accepted | OAuth 2.0 only, so the row sets every endpoint explicitly. `sub` is the numeric GitHub account id. Uses the kinotic-ai GitHub App's user-authorization OAuth credential — the same credential that verifies installation ownership when an org links GitHub, so the row must never point at a different OAuth client. The app needs the "Email addresses: read-only" permission, and its scope param is ignored (GitHub App permissions govern access) |
...
Each server mounts the routes of the handlers its application class imports, on its own gateway port (org `58503`, system `58504`, app `58505` by default):

| Handler | Org server | System server | App server |
|---|---|---|---|
| `OrganizationLoginHandler`, `OrganizationSignupHandler`, `InviteHandler`, `DeviceAuthorizationHandler`, `GitHubWebhookHandler` | ✓ | | |
| `SystemLoginHandler` | | ✓ | |
| `ApplicationLoginHandler` | | | ✓ |
| `SessionEndpointHandler`, `OAuthServerHandler` | ✓ | ✓ | ✓ |
| `McpJsonRpcHandler` | ✓ | ✓ | |

The app server serves every application at an API host of its own, `<organizationId>--<applicationId>` under `kinotic.domain.appApiBaseUrl` (see [Configuration](/platform/configuration#application-api-hosts)). Its login and OAuth routes act for the application whose host the request is addressed to and answer `404` on any other host, and that host is the application's OAuth issuer.
...
| `POST` | `/api/auth/org/signup/complete` | `OrganizationSignupHandler` | Verify token + orgName + password; creates Organization + admin ParticipantIdentity; establishes session |
...
| `POST` | `/api/auth/org/signup/social/complete` | `OrganizationSignupHandler` | Consume PendingSignUp; create Org + ParticipantIdentity; establishes session |
| `POST` | `/api/auth/system/login` | `SystemLoginHandler` | System operator email + password; establishes session |
| `GET`  | `/api/auth/app/login/providers` | `ApplicationLoginHandler` | Enabled OIDC configs the application references |
| `POST` | `/api/auth/app/login/lookup` | `ApplicationLoginHandler` | App-scoped email-first lookup; `403` from a page that is not one of the application's UIs |
| `POST` | `/api/auth/app/login` | `ApplicationLoginHandler` | App-scoped email + password; establishes session; `403` from a page that is not one of the application's UIs |
| `GET`  | `/api/auth/app/login/oidc/callback/:configId` | `ApplicationLoginHandler` | App IdP returns here; establishes session, 302 to the page that started the login |
| `GET` | `/.well-known/oauth-authorization-server` | `OAuthServerHandler` | RFC 8414 authorization-server metadata |
| `GET` | `/.well-known/oauth-protected-resource[/mcp]` | `McpJsonRpcHandler` | RFC 9728 resource metadata for `/mcp` |
| `POST` | `/mcp` | `McpJsonRpcHandler` | MCP JSON-RPC endpoint (see [MCP Tools](/platform/mcp-tools)) |
| `GET` | `/api/auth/oauth/authorize` | `OAuthServerHandler` | PKCE authorization-code flow; redirects to the `/oauth/consent` page on the server's UI, on the app server the application's primary UI (`400` until one is chosen) |
| `GET` | `/api/auth/oauth/request/:requestId` | `OAuthServerHandler` | The request awaiting consent, for the consent page's signed-in user |
| `POST` | `/api/auth/oauth/approve` | `OAuthServerHandler` | Approves the request as the consent page's signed-in user; answers the client redirect URL carrying the code |
| `POST` | `/api/auth/oauth/deny` | `OAuthServerHandler` | Denies the request; answers the client redirect URL carrying `error=access_denied` |
| `POST` | `/api/auth/oauth/device_authorization` | `DeviceAuthorizationHandler` | RFC 8628 device grant — issue device/user codes; requires the CLI's `client_id` |
| `POST` | `/api/auth/oauth/device/approve` | `DeviceAuthorizationHandler` | Approves the user code the `/device` page's signed-in user confirmed |
...
| `POST` | `/api/auth/invite/accept` | `InviteHandler` | Accept by setting a password; establishes an organization member's session, while an application member signs in at the application |
| `POST` | `/api/auth/invite/oidc/start/:configId` | `InviteHandler` | Accept via OIDC; redirects to the IdP |
| `GET`  | `/api/auth/invite/oidc/callback/:configId` | `InviteHandler` | IdP returns here; accepts the invite, establishes an organization member's session |
| `GET`  | `/api/auth/me` | `SessionEndpointHandler` | `204` if the session cookie holds a login for the calling page, else `401` |
| `POST` | `/api/auth/logout` | `SessionEndpointHandler` | Ends the calling page's login; the session ends once no page's login remains |
| `POST` | `/api/github/webhook` | `GitHubWebhookHandler` | GitHub App webhook, verified against the App's webhook secret |
