...
GitHub follows the same flow with one difference in step 6: GitHub is OAuth 2.0 only and issues no id_token, so `OidcFlowOrchestrator` builds the identity claims from the GitHub API instead — `GET /user` supplies `sub` (the numeric account id) and the display name, and `GET /user/emails` supplies the email plus its `verified` flag (primary address preferred). Issuer, audience, and nonce validation don't apply (there is no token to carry them); the state match and the direct TLS code exchange are the security boundary.
...
OIDC is a standard, but providers diverge on a few details. The validation helpers in `OAuth2Util` (`isIssuerValid`, `isEmailVerified`) handle these declaratively — the provider key on `OidcConfiguration` selects the right behaviour. No provider needs handler-level branching; GitHub, the one non-OIDC provider, is branched inside `OidcFlowOrchestrator` (no discovery, OAuth scopes, GitHub-API claims) and every handler consumes the same claims shape.

| Provider key | `iss` shape | `email_verified` claim | Other notes |
|---|---|---|---|
| `github` | No id_token — identity is fetched from the GitHub API with the exchanged access token, so issuer/nonce validation doesn't apply | Synthesized from `GET /user/emails` — the primary verified address is preferred, any verified address accepted | OAuth 2.0 only. `sub` is the numeric GitHub account id. Uses the kinotic-ai GitHub App's user-authorization OAuth credential; the app needs the "Email addresses: read-only" permission, and its scope param is ignored (GitHub App permissions govern access) |
...
`isEmailVerified`, `isIssuerValid`, and the GitHub branch in `OidcFlowOrchestrator` are the only places these differences live. Adding a new provider that follows the standard set of conventions doesn't require code changes; only providers with non-standard quirks (Apple's first-login-only email, Microsoft's `/common` issuer template, GitHub's absent id_token) need to be classified explicitly there.
