...
Each server also has a zone partition, the zones it hosts and the zones it reaches. A node advertises a service
...
| Server | Hosts | Reaches |
|--------|-------|---------|
| org | `management-api` | `management-api`, `system-api`, `app-api` |
| system | `system-api`, `management-api` | `system-api`, `management-api` |
| app | `app-api`, `app.*` | `app-api`, `app.*` |

The service directory and listener monitors still see every registration in the cluster, so a service hosted
by one server shows as online from any other.
...
| `srv://system-api~org.kinotic.system.api.services.workload.WorkloadOrchestrationService` | A platform-internal service |
