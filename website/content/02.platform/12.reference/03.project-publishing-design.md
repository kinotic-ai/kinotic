...
- **The project SBOM.** The deploy job's sixth task, **Generate SBOM**, compares the
  `dependencyHash` the sync VM reported with the one `ProjectDeployment.sbom` was generated from
  ...
  (`OrganizationStorageService.issueWriteUrl`), records it on the deployment with `recordSbom` as
  the sync machine, whose secret is reissued for it, and then deletes the files of other commits.
  The portal's
  ...
  (`OrganizationStorageService.issueReadUrl`). A project's deletion deletes the SBOM with its
  deployment; its files stay in the organization's directory.
