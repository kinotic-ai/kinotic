...
- **The project SBOM.** A project has one commit and one dependency hash, both on
  ...
  (`OrganizationStorageService.issueSbomWriteUrl`), and records it with `recordSbom` as the sync
  ...
  signs itself (`OrganizationStorageService.issueSbomReadUrl`). A project's deletion deletes the SBOM
