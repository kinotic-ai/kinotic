...
`disableProvisioner`. The SBOMs add `kinotic.systemApi.organizationStorage.blobEndpoint`, the
one account every organization's files are partitioned in (`organizations/<org>/<use>/`), and
`kinotic.systemApi.deployment.disableSbom`. Nothing is provisioned when an organization is
...
- **The project SBOM.** The deploy job's sixth task, **Generate SBOM**, compares the
  ...
  read SAS for the one file (`DeploymentOperationsProxy.issueSbomUrl`). A project's deletion
  deletes its `ProjectSbom` row; its files stay in the organization's directory.
