...
| 6 | Generate SBOM | when `ProjectDeployment.sbom` is absent, which `recordArtifacts` makes it whenever the reported `dependencyHash` changes: a write SAS for `organizations/<org>/sboms/<project>.cdx.json` → SBOM VM runs cdxgen, uploads `<commit>.cdx.json`, records it through `ProjectArtifactService.recordSbom` → verify the record names the commit; otherwise nothing runs. Absent when `kinotic.systemApi.deployment.disableSbom` is set. `Store.state(SBOM).wire()` |
...
- **The project SBOM.** A project has one commit and one dependency hash, both on
  `ProjectDeployment.artifacts`; `ProjectDeployment.sbom` (component count, generated) is the SBOM
  of those dependencies, and `recordArtifacts` drops it when a sync reports another hash. The
  deploy job's sixth task, **Generate SBOM**, runs `project-sbom-<projectId>` under
  `DeployTarget.sbomWorkloadId` only when the deployment has no SBOM: `generate-sbom.ts` runs
