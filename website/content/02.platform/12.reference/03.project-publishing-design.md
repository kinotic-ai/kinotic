...
| 6 | Generate SBOM | while `ProjectDeployment.sbomGenerated` is false, which `recordArtifacts` makes it whenever the reported `dependencyHash` changes: a write SAS for `organizations/<org>/sboms/<project>.cdx.json` → SBOM VM runs cdxgen, uploads the document, records it through `ProjectArtifactService.recordSbom` → verify the flag is set; otherwise nothing runs. Absent when `kinotic.systemApi.deployment.disableSbom` is set. `Store.state(SBOM).wire()`, whether the run generated the SBOM |
...
`disableProvisioner`. The SBOMs add `kinotic.managementApi.organizationsStorageEndpoint`,
the one account every organization's files are partitioned in (`organizations/<org>/<use>/`),
and `kinotic.systemApi.deployment.disableSbom`. Nothing is provisioned when an organization is
created: its directory exists once a file is written under it.
