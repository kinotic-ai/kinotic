...
| 6 | Generate SBOM | when `ProjectArtifacts.dependencyHash` differs from the `ProjectSbom`'s: a write SAS for `organizations/<org>/sboms/<project>` → SBOM VM runs cdxgen, uploads `<commit>.cdx.json`, records it through `ProjectArtifactService.recordSbom` → verify the record names the commit; otherwise nothing runs. Absent when `kinotic.systemApi.deployment.disableSbom` is set. `Store.state(SBOM).wire()` |
...
- **The project SBOM.** The deploy job's sixth task, **Generate SBOM**, compares the
  ...
  (`OrganizationStorageService.issueWriteUrl`), records it with `recordSbom` as the sync machine,
  whose secret is reissued for it, and then deletes the files of other commits. The portal's
  SBOM page reads the document through `ProjectSbomService.findDocumentUrl`, a fifteen-minute
