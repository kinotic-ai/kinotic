...
`findAllForProject` and `remove`, and UIs also have `findAllForApplication`; removal is the
only path that destroys anything.
...
(`@acme/admin` is `admin`), a single zone label, and a UI's without `--`; a name breaking
either rule fails the deploy naming the package. The directory name never matters. The sync VM reports what it found
...
The hostname label is `{org}--{app}--{ui}` under `sitesDomain` (`apps.kinotic.ai`), minted at
first publish, stored as `UiDeployment.id` and looked up by hostname. No organization id,
application id or UI name contains `--`, and the organization id `xn` is reserved so no label
starts with DNS's `xn--`, so a label names exactly one UI of one application. The
repository's create enforces uniqueness: a UI name another project of the application already
published fails the publish. A label is at most 63 characters, else the publish fails naming
the organization and application.

## Auth

`*.apps.kinotic.ai` is same-site with an application's API host, so the app server's session
cookie flows, and the app server's CORS pattern admits the sites domain. The app server's session
cookie is named `__Host-kinotic-app-session` (Secure and `Path=/` are already set), `SameSite=Lax` unless
...
`DomainProperties.resolveAppApiUrl(AppHost)` (`https://<org>--<app>.<appApiBaseUrl domain>`),
...
- **UIs built in the sync VM.** After the entity sync, `sync.ts` runs `bun run build` in
  ...
  `DomainProperties.resolveAppApiUrl(AppHost)`, and fails the run naming a UI whose build leaves no
  `dist/index.html`.
...
- **The publish task.** The deploy job's fifth task, **Publish UIs**, mints the label
  `{org}--{app}--{ui}` for each UI published for the first time, then runs
...
- **Sites in the console.** `UiDeployment.url` is minted with the
  ...
  reports and the commit it serves, with removal. Each server's session cookie is `__Host-kinotic-<server>-session`,
  named once in `ApiGatewayVertcleFactory` from the server's `ZonePartition` and handed to the
  gateway's session handler and to the `SessionBinding` that resolves each page's login from it.
