...
machine identity, with the `dependencyHash` of the checkout (a SHA-256 of `bun.lock` and of the
cdxgen version), and the server records it on `ProjectDeployment` with the commit it was found
in; the server applies the same name rule to what it is told.
...
| 3 | Resolve artifacts | binds the `ProjectArtifacts(microservices, uis, dependencyHash)` the sync VM reported for the commit into the run; fails when the record names another commit. `Store.state(ARTIFACTS).wire()` |
...
| 6 | Generate SBOM | when `ProjectArtifacts.dependencyHash` differs from the `ProjectSbom`'s: a write SAS for `organizations/<org>/sboms/<project>` → SBOM VM runs cdxgen, uploads `<commit>.cdx.json` → once it succeeded, the job saves the `ProjectSbom` with the commit and the reported hash; otherwise nothing runs. Absent when `kinotic.systemApi.deployment.disableSbom` is set. `Store.state(SBOM).wire()` |
...
`disableProvisioner`. The SBOMs add `kinotic.managementApi.organizationStorage.blobEndpoint`,
...
- **The project SBOM.** The deploy job's sixth task, **Generate SBOM**, compares the
  `dependencyHash` the sync VM reported with the one the `ProjectSbom` row was generated from
  and runs `project-sbom-<projectId>` under `DeployTarget.sbomWorkloadId` only when they differ:
  `generate-sbom.ts` runs cdxgen over the read-only checkout, uploads the CycloneDX document into
  the project's directory of the organization storage account through a one-hour directory SAS
  (`OrganizationStorageService.issueWriteUrl`), its only credential, and then deletes the files
  of other commits; once the workload has succeeded, the job saves the `ProjectSbom` row with the
  commit and the hash the sync VM reported. The portal's
  SBOM page reads the document through `ProjectService.findSbomDocumentUrl`, a fifteen-minute
  read SAS for the one file that management-api signs itself
  (`OrganizationStorageService.issueReadUrl`). A project's deletion deletes its `ProjectSbom`
  row; its files stay in the organization's directory.
