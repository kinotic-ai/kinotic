...
Door, the sites and organization storage accounts, and email in Azure and is designed to
migrate its organizations to Kinotic Cloud. See [Development Server](/platform/development-server).
...
applying the rest. The development profile sets `kinotic.systemApi.disableAzureStorage`
instead (see [Configuration](/platform/configuration#without-azure-storage)).
...
`organizations-storage.tf` calls `deployment/terraform/azure/modules/organizations-storage`,
which creates the one account every organization's files are kept in, `st<prefix>orgs`: an
account with a hierarchical namespace, so a URL can be scoped to one directory, and anonymous
access off, its `organizations` container, a CORS rule admitting `GET` and `HEAD` from the
portal's origin, and Storage Blob Data Contributor for the kinotic-server identity, with which
the server signs the URLs the SBOM workloads write through and the portal reads through. `kinotic.tf` passes the account's blob endpoint as
`KINOTIC_MANAGEMENTAPI_ORGANIZATIONSSTORAGEENDPOINT` (see
[Configuration](/platform/configuration#organization-storage)). Nothing is created when an
organization is: its directory exists once a file is written under it. The `dev` and
`dev-server` roots create the same account for their environments.
