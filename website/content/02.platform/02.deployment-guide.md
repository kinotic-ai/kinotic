...
which creates the one account every organization's files are kept in, `st<prefix>orgs`: a
Data Lake Gen2 account with anonymous access off, its `organizations` container, a CORS rule
admitting `GET` and `HEAD` from the portal's origin, and Storage Blob Data Contributor for the
kinotic-server identity, with which the server signs the URLs the SBOM workloads write through
and the portal reads through. `kinotic.tf` passes the account's blob endpoint as
`KINOTIC_SYSTEMAPI_ORGANIZATIONSTORAGE_BLOBENDPOINT` (see
