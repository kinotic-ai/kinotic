...
- **The `aud` claim must match the entry point.** Each entry point accepts exactly one audience — `kinotic` at the STOMP handshake, `kinotic-mcp` at `POST /mcp` — so a token issued for one surface is rejected at every other, and any non-Kinotic JWT (even a valid token from a trusted IdP) is rejected because it carries neither value.
