...
A second check is minted but not yet enforced: every token carries an `aud` claim naming the surface its grant serves, and no entry point verifies it, so a token issued for one surface currently reaches every other.
