...
| `kinotic.systemApi.deployment.serverHost` / `serverPort` / `serverUseSsl` | the server's LAN IPv4, `443`, `true` | Workloads on the nodes reach the gateway across the LAN; a `CLOUD_HYPERVISOR` node rejects a hostname here. The guest trusts the Let's Encrypt chain |
