...
(Front Door, the sites storage account, the organization storage account, Communication
Services email, the DNS zone), what the migration path needs (a Key Vault, a snapshot
container), and the portal and the system console, static files served by the same Front
Door as the published sites.
...
`DefaultAzureCredential` takes before anything else. A `dev-server` root is that root plus
...
| Property | Value | Why |
|---|---|---|
| `kinotic.domain.appBaseUrl` | `https://dev-portal.kinotic.ai` | The portal, served by Front Door from the sites account |
| `kinotic.domain.apiBaseUrl` | `https://dev-api.kinotic.ai` | REST, STOMP, MCP, and the OIDC redirect URIs; `issuerBaseUrl` falls back to it |
| `kinotic.apiGateway.stompPort` | `443` | The api-gateway port is the public one: a consumer router forwards a port to the same port only, and an LXC autodev hook lowers the container's unprivileged port floor so the server's user binds it |
| `kinotic.apiGateway.webServer.enabled` | `false` | Nothing is served from the jar's webroot; the API port is the only one exposed, as in the cluster |
| `kinotic.apiGateway.ssl.enabled` / `certPath` / `keyPath` | `true`, the two certbot PEMs | Vert.x terminates TLS, no reverse proxy, matching KinD and Azure |
...
| `kinotic.systemApi.disableAzureStorage` | `false` | Front Door serves the published UIs from the sites account, and each deployment's SBOM is kept in the organization storage account |
| `kinotic.systemApi.uiDeployment.sitesDomain` / `sitesStorageEndpoint` | `apps-dev.kinotic.ai`, the development server's sites account | The `dev-server` root's outputs, as the `dev` root emits them |
| `kinotic.managementApi.organizationsStorageEndpoint` | the development server's organization storage account | The `dev-server` root's output; each deployment's SBOM is written there and the portal reads it from there, so the account's CORS rule names `https://dev-portal.kinotic.ai` |
| `kinotic.systemApi.deployment.serverHost` / `serverPort` / `serverUseSsl` | `dev-api.kinotic.ai`, `443`, `true` | Workloads dial the gateway by the name its certificate carries; each node pins that name to the server's LAN address in its `/etc/hosts`, which its resolver answers guests from and the vm-manager admits in the name's egress set, so the traffic stays on the LAN. The guest trusts the Let's Encrypt chain |
| `kinotic.systemApi.deployment.syncAllowedHosts` | `github.com`, `registry.npmjs.org` | The node's resolver admits what it answers for each name, so the same names serve `BOXLITE` development and the `CLOUD_HYPERVISOR` nodes here. The SBOM VM is admitted to them too, for the registry's license lookups |
| `kinotic.apiGateway.cors.allowedOriginPattern` / `allowCredentials` | the portal, the console, and `https://[a-z0-9-]+\.apps-dev\.kinotic\.ai`; `true` | The `kubernetes` profile hardcodes `kinotic.ai` and is not active here |
| `kinotic.managementApi.github.appPrivateKey` / `webhookSecret` | from `secrets.yml` | The cloud's App; its webhook URL points at `https://dev-api.kinotic.ai/api/github/webhook` |
...
| `KINOTIC_SERVER_HOST` / `KINOTIC_SERVER_PORT` / `KINOTIC_SERVER_USE_SSL` | the server's LAN IPv4, `443`, `true` |
| `KINOTIC_CLIENT_ID` / `KINOTIC_CLIENT_SECRET` | a SYSTEM-scope machine created in the system console at **Members → Machines**; client credentials are the non-Azure machine path |
| `KINOTIC_WORKLOAD_DATA_DIR` | `/var/lib/kinotic/workloads` |
| `KINOTIC_WORKLOAD_DNS` | the node's own dnsmasq on the docker bridge address, `172.17.0.1`, permitted on port 53 for every workload; it is what lets a hostname in a workload's egress policy be enforced |
...
| `modules/organizations-storage` | Storage account `stkinoticdevorgs`, the organization storage account: each project's SBOM under `organizations/<org>/sboms/`, readable from `dev-portal.<zone>` |
| Storage account `stkinoticdevsnapshots` | The ES snapshot repository's container |
| Front Door domains `dev-portal.<zone>`, `dev-console.<zone>` | The portal and the system console, served from `sites/<hostname>/` in the sites account by the route and rule set every published site uses, on managed certificates; `deploy-ui.sh` builds both with `--mode dev-server` and uploads them |
| DNS `dev-api.<zone>` A record | The host's public address; `kinotic-dyndns.timer` on the host rewrites it when the ISP changes the address |
| Service principal `kinotic-dev-server` | Storage Blob Data Contributor on the sites and organization storage accounts, Contributor on the email service, Key Vault Secrets Officer on the server vault, DNS Zone Contributor on the zone for certbot and the address updater |
...
| Project SBOMs | `kinotic_project_deployment.sbomGenerated`, the documents in the development server's organization storage account | Cleared; each project's redeploy generates its SBOM into the cloud's account |
...
3. Delete the restored `kinotic_vm_node` and `kinotic_workload` rows and clear `nodeId`,
   `hostDir`, `syncWorkloadId`, `uiPublishWorkloadId`, `sbomWorkloadId`, and `sbomGenerated` on every
   ...
   next deployment. A restored `sbomGenerated` would keep the redeploy from generating the document,
   which the cloud's organization storage account does not hold.
   A re-home operation is the durable version of this step.
...
6. Re-run the deployment of every project, which places it on a cloud node, republishes its
   UIs under `apps.kinotic.ai`, and generates its SBOM.
