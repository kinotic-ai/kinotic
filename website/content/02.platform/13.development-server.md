...
| `kinotic.managementApi.entityDefinition.numberOfShards` / `numberOfReplicas` | `1` / `1` |
...
| `kinotic.systemApi.deployment.orgServer` and `appServer`: `host` / `port` / `useSsl` | `dev-api.kinotic.ai`, `443`, `true` for both, since the one server is both | Workloads dial the gateway by the name its certificate carries; each node pins that name to the server's LAN address in its `/etc/hosts`, which its resolver answers guests from and the vm-manager admits in the name's egress set, so the traffic stays on the LAN. The guest trusts the Let's Encrypt chain |
| `kinotic.systemApi.deployment.syncAllowedHosts` | `github.com`, `registry.npmjs.org` | The node's resolver admits what it answers for each name, so the same names serve `BOXLITE` development and the `CLOUD_HYPERVISOR` nodes here |
...
| Key Vault `kv-kinotic-dev` | The servers' secret storage; the principal holds Key Vault Secrets Officer, as `cluster/keyvault.tf` grants the workload identity |
...
| DNS A records `dev-api`, `dev-system-api`, `dev-apps-api` and `*.dev-apps-api` under the zone | The host's public address, for the org server, the system server, and the app server with every application's API host, `<organizationId>--<applicationId>.dev-apps-api.<zone>`; `kinotic-dyndns.timer` on the host rewrites them when the ISP changes the address |
| Service principal `kinotic-dev-server` | Storage Blob Data Contributor on the sites account, Contributor on the email service, Key Vault Secrets Officer on the server vault, DNS Zone Contributor on the zone for certbot and the address updater |
