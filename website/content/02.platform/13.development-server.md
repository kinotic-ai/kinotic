...
| Project SBOMs | `kinotic_project_deployment.sbom`, the documents in the development server's organization storage account | Cleared; each project's redeploy generates its SBOM into the cloud's account |
...
3. Delete the restored `kinotic_vm_node` and `kinotic_workload` rows and clear `nodeId`,
   `hostDir`, `syncWorkloadId`, `uiPublishWorkloadId`, `sbomWorkloadId`, and `sbom` on every
   ...
   next deployment. A restored `sbom` names a document the cloud's organization storage account
   does not hold, and its dependency hash would keep the redeploy from generating one.
