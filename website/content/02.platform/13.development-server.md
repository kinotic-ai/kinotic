...
| `kinotic.systemApi.organizationStorage.blobEndpoint` | the development server's organization storage account | The `dev-server` root's output; each deployment's SBOM is written there and the portal reads it from there, so the account's CORS rule names `https://dev-portal.kinotic.ai` |
