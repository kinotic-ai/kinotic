...
| `kinotic.managementApi.organizationStorage.blobEndpoint` | the development server's organization storage account | The `dev-server` root's output; each deployment's SBOM is written there and the portal reads it from there, so the account's CORS rule names `https://dev-portal.kinotic.ai` |
...
| `modules/organization-storage` | Storage account `stkinoticdevorgs`, the organization storage account: each project's SBOM under `organizations/<org>/sboms/`, readable from `dev-portal.<zone>` |
...
| Project SBOMs | `kinotic_project_sbom` rows, their documents in the development server's organization storage account | The rows are deleted; each project's redeploy generates its SBOM into the cloud's account |
...
3. Delete the restored `kinotic_vm_node`, `kinotic_workload` and `kinotic_project_sbom` rows
   and clear `nodeId`, `hostDir`, `syncWorkloadId`, `uiPublishWorkloadId`, and `sbomWorkloadId`
   on every `kinotic_project_deployment` row. `resolveTarget` reuses an existing deployment's
   node without checking it exists, so a row still pointing at the development node would fail
   its next deployment. A restored SBOM row names a document the cloud's organization storage
   account does not hold, and its dependency hash would keep the redeploy from generating one.
