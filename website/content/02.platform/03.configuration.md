...
| `runtimeMemoryMb` | `1024` | Memory of each microservice's runtime VM, and of each site publish or removal VM |

The CPU and root filesystem of these VMs are the same in every environment; what differs between nodes is how many fit. The sync VM gets half a core and a 2 GB root filesystem, where the package manager keeps its cache; each runtime VM, and each site publish or removal VM, gets half a core and a 512 MB root filesystem — the checkout is a read-only mount, so the root holds only what the microservice writes. The disk caps are enforced where the node's filesystem carries project quotas.
