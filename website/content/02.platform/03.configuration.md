...
per deployment, which connects to the org server, and one long-lived runtime VM per
microservice, which connects to the app server, configured under
`kinotic.systemApi.deployment.*`:

| Property | Default | Meaning |
|---|---|---|
| `orgServer.host` | **required** | Host the sync workload uses to reach the org server (`KINOTIC_SERVER_HOST` in the guest), and the one destination its egress policy always permits. A server has no advertised address of its own, so startup fails without it. An IPv4 address or a hostname, on either provider |
| `orgServer.port` | `58503` | Port the sync workload uses to reach the org server |
| `orgServer.useSsl` | `false` | Whether the sync workload reaches the org server over TLS |
| `appServer.host` | **required** | Host the runtime workloads use to reach the app server, and the one destination their egress policy always permits. An IPv4 address or a hostname, on either provider |
| `appServer.port` | `58505` | Port the runtime workloads use to reach the app server |
| `appServer.useSsl` | `false` | Whether the runtime workloads reach the app server over TLS |
...
| `syncAllowedHosts` | `[]` | Destinations (IPv4 addresses, CIDRs, or hostnames) the sync workload may reach beyond the org server — typically `github.com` and `registry.npmjs.org`, so `git fetch` and `bun install` work under default-deny egress |
| `runtimeAllowedHosts` | `[]` | Destinations (IPv4 addresses, CIDRs, or hostnames) the runtime workloads may reach beyond the app server |
...
signs an application's users in only from pages that are the application's own UIs. Both are configured under `kinotic.domain.*`:

| Property | Default | Meaning |
|---|---|---|
| `appApiBaseUrl` | `http://localhost:58505` | Scheme, domain and optional port every application's API host is a label under: with `https://apps-api.kinotic.ai`, application `orders` of organization `acme` is reached at `https://acme--orders.apps-api.kinotic.ai` |
| `localAppUiOriginPattern` | — | Origins admitted as a UI of every application, for UIs a developer serves from their own machine; the app server's development profile sets `http:\/\/localhost:\d+`. Unset, an application's login is made only from its published sites, `<organizationId>--<applicationId>--<uiName>` under the sites domain |
