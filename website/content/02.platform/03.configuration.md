...
| `syncAllowedHosts` | `[]` | Destinations (IPv4 addresses, CIDRs, or hostnames) the sync workload may reach beyond the gateway — typically `github.com` and `registry.npmjs.org`, so `git fetch` and `bun install` work under default-deny egress |
