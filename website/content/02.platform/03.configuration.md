...
| `disableSbom` | `false` | When true a deployment generates no SBOM, for an environment without the [organization storage](#organization-storage) account SBOMs are kept in; the development and test profiles set it |
...
`organizations/<organizationId>/sboms/<projectId>.cdx.json`. The account is configured under
`kinotic.managementApi.organizationStorage.*`:

| Property | Default | Meaning |
|---|---|---|
| `blobEndpoint` | — | Blob endpoint of the organization storage account terraform creates, e.g. `https://stkinoticproductionorgs.blob.core.windows.net/`. Required |

The endpoint is validated at boot, so the development and test profiles, which have no account
and set `kinotic.systemApi.deployment.disableSbom`, set it to a placeholder. In the Azure
deployment terraform creates the account with anonymous access off and passes its endpoint to the
server (see the [deployment guide](/platform/deployment-guide)); the `local` profile of the
[contributing guide](/platform/contributing#publishing-uis-against-azure) names a developer's
own. The server never reads or writes the account itself: it holds Storage Blob Data Contributor
on it only to sign short-lived URLs, each scoped to one directory or one file, that workloads
write through and the portal reads through. The account's CORS rules admit the portal's
