...
Every destination comes from the workload's own `network.allowedHosts`, including the api-gateway — the server sets it there, because only the server knows where the gateway is. The node adds the destinations the workload cannot know: the resolver it was given, and its own trace endpoint for a workload that elects `tracing` (see [Workload Traces](/platform/observability#workload-traces)).
...
`mode: DISABLED` leaves no interface to publish a port on or to reach the node's trace endpoint through, so a node refuses a workload declaring it together with `portMappings` or `tracing` — on either provider — rather than starting one whose published ports or traces cannot work.
