...
keeps one storage account per organization, provisioned on first need by the management
module and configured under `kinotic.managementApi.organizationStorage.*`:
...
| `subscriptionIds` | `[]` | The Azure subscriptions accounts are spread over; an organization's account is created in one of them and stays there. Required unless the provisioner is disabled |
| `resourceGroup` | — | The resource group, present in every listed subscription, holding the accounts and their private endpoints. Required unless the provisioner is disabled |
| `location` | — | The Azure region the accounts are created in. Required unless the provisioner is disabled |
| `privateEndpointSubnetId` | — | Id of the subnet in the platform VNet each account's private endpoint is placed in. Required unless the provisioner is disabled |
| `privateDnsZoneId` | — | Id of the `privatelink.blob.core.windows.net` private DNS zone each account is registered in. Required unless the provisioner is disabled |
...
The settings are checked when a deployment first needs storage, not at startup, so a server
that never publishes a UI runs without them. In the Azure deployment, terraform creates the
resource group, subnet and private DNS zone and passes their ids to the server (see the
...
Azure Front Door, configured under `kinotic.managementApi.uiDeployment.*`:
...
| `dnsZoneId` | — | Id of the Azure DNS zone holding `sitesDomain`, where each site's CNAME and validation TXT are written. Required unless the provisioner is disabled |
| `frontDoorProfileId` | — | Id of the Front Door Standard profile every site is served through. Required unless the provisioner is disabled |
| `frontDoorEndpointHostName` | — | Host name of the profile's endpoint, the target of every site's CNAME. Required unless the provisioner is disabled |

Like organization storage, the settings are checked when a deployment first publishes a UI.
In the Azure deployment terraform creates the profile and endpoint and passes them to the
