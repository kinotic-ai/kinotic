...
A node also denies every workload the cloud metadata endpoint (`169.254.169.254`) and the Azure WireServer's control ports (`168.63.129.16`), which is what keeps a guest from reading the host's own credentials. An `allowedHosts` entry covering either — naming it outright, or a range such as `169.254.0.0/16` that contains it — is refused, and the workload fails to start rather than being given access the node denies everything else.
