...
`sizeLimitMb` applies to writable mounts; a mount declared `readOnly` is already unwritable. It is enforced by an XFS project quota, so the host filesystem refuses the write once the cap is reached whatever the workload does — which requires the mount to live on an XFS filesystem mounted with `prjquota`, on a node running either provider. A node whose filesystem cannot enforce a cap logs the mount it could not cap and runs the workload uncapped; a `CLOUD_HYPERVISOR` node also reports the condition on its heartbeat and stops being given workloads. A workload's logs occupy at most `maxSizeMb * (maxFiles + 1)`, and the limits are applied by the VM provider rather than by the workload, so a workload cannot raise its own ceiling.
