...
A node that cannot enforce a hostname — no dnsmasq listening for its workloads, no ipset — refuses a workload declaring one rather than starting it with the name dropped from its policy.
...
| `runtimeAllowedHosts` | `[]` | Destinations (IPv4 addresses, CIDRs, or hostnames) the runtime workload may reach beyond the gateway |
