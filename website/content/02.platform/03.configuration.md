...
A node denies every workload the cloud metadata endpoint (`169.254.169.254`) and the Azure WireServer's control ports (`168.63.129.16`), which is what keeps a guest from reading the host's own identity. A workload may still be granted one, but only by naming the address exactly:

| `allowedHosts` entry | Result |
|---|---|
| `169.254.169.254/32` or `169.254.169.254` | Granted, and logged on the node |
| `169.254.0.0/16`, `0.0.0.0/0` | Refused — the range reaches it without naming it, and the workload fails to start |

Only a system participant can reach the service that carries a workload's policy, so an address named outright is a decision the platform made. A range that merely contains one is not, which is what stops a policy meaning "the internet" from handing over the host's identity as a side effect. Note that `0.0.0.0/0` is therefore not a way to express unrestricted egress.
...
Each node re-checks these on every heartbeat and reports what it can no longer guarantee. A node reporting problems moves to `DRAINING`, so the orchestrator places no further workloads on it, and back to `ONLINE` once it reports none. The workloads already there keep running — a node that stopped enforcing a limit is unfit to take on more, not required to drop what it has. `VmNode.healthMessage` carries the reason, and the system console shows it on the node.
