...
`cpus` is in cores and may be a fraction: `0.5` is half a core, enforced as a CPU quota on the VM on `CLOUD_HYPERVISOR` nodes (a `BOXLITE` node boots the VM with the whole core the share is part of). The node's `availableCpus` moves by the same fraction, so a 4-core node holds eight half-core workloads.

The node's record keeps a ledger of the room each workload holds, one entry per workload id, and `availableCpus`, `availableMemoryMb` and `availableDiskMb` are the totals less what the ledger holds. A run holds everything the workload is sized for. When a run ends — the VM stops, fails, or the node reports it gone — its CPU and memory return to the node while its disk stays held, since a stopped VM keeps its disk until the workload is destroyed; a restart takes the CPU and memory again, and fails when the node no longer has them. Destroying the workload returns everything. Reserving or releasing the same workload twice changes nothing the second time, so a stop and the node's own report of the same end cannot return the room twice. A node that registers again rebuilds the ledger from the workload records placed on it, so a run the node ended while the server was away is still accounted for.

A workload that runs to completion (`Workload.detached` false) is destroyed by the orchestrator as soon as its reply arrives, whatever its outcome: its VM, its room on the node and its record are gone, and its logs stay in the organization's log store under its id. Only a detached workload keeps its VM after a run.
...
| `syncCpus` | `1` | CPU of the sync VM, in cores — it compiles the project's entity sources and builds the UIs, which is where a deployment spends its CPU |
...
| `syncDiskSizeMb` | `2048` | Size cap on the sync VM's root filesystem, where the package manager keeps its cache, enforced the same way |
| `runtimeCpus` | `0.5` | CPU of each microservice's runtime VM, in cores; a fraction is a share of one core. The site publish and removal workloads are sized the same way |
| `runtimeMemoryMb` | `1024` | Memory of each microservice's runtime VM |
| `runtimeDiskSizeMb` | `512` | Size cap on each runtime VM's root filesystem; the checkout is a read-only mount, so the root holds only what the microservice writes |
