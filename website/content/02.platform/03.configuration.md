...
|---|---|
| `KINOTIC_WORKLOAD_DNS` (vm-manager) | Resolver each workload is given, permitted on port 53. On a provisioned `CLOUD_HYPERVISOR` node, the docker bridge address the node's dnsmasq listens on |
...
| `['registry.npmjs.org']` | Whatever the node's resolver answers for that name, minus what the node denies |
...
|---|---|---|
| `serverHost` | **required** | Host the deployed workloads use to reach the api-gateway (`KINOTIC_SERVER_HOST` in the guest), and the one destination every workload's egress policy always permits. The server has no advertised address of its own, so startup fails without it. An IPv4 address or a hostname, on either provider |
...
| `workloadRunnerImage` | `kinoticai/workload-runner:latest` | OCI image every deployment workload runs — the checkout/sync, UI publish and SBOM entrypoints and the microservice supervisor |
| `syncAllowedHosts` | `[]` | Destinations (IPv4 addresses, CIDRs, or hostnames) the sync workload may reach beyond the gateway — typically `github.com` and `registry.npmjs.org`, so `git fetch` and `bun install` work under default-deny egress. The SBOM workload may reach them too, to look up each package's license in the registry |
| `runtimeAllowedHosts` | `[]` | Destinations (IPv4 addresses, CIDRs, or hostnames) the runtime workloads may reach beyond the gateway |
...
The files the platform keeps on behalf of organizations live in one storage account per
environment, partitioned by organization and then by use: `organizations/<organizationId>/<use>/`
in the account's `organizations` container. Today the one use is each project's SBOM,
`organizations/<organizationId>/sboms/<projectId>.cdx.json`. The server is told the account's
blob endpoint:

| Property | Default | Meaning |
|---|---|---|
| `kinotic.managementApi.organizationsStorageEndpoint` | — | Blob endpoint of the organization storage account terraform creates, e.g. `https://stkinoticproductionorgs.blob.core.windows.net/`. Required |

The endpoint is validated at boot, so an environment
[without Azure storage](#without-azure-storage) still sets it, to a placeholder. In the Azure
deployment terraform creates the account with anonymous access off and passes its endpoint to
the server (see the [deployment guide](/platform/deployment-guide)); the `local` profile of the
[contributing guide](/platform/contributing#publishing-uis-against-azure) names a developer's
own. The server never reads or writes the account itself: it holds Storage Blob Data
Contributor on it only to sign short-lived URLs, each scoped to one directory or one file, that
workloads write through and the portal reads through. The account's CORS rules admit the portal's
origins, so a browser holding such a URL reads the file directly.

## Without Azure storage

An environment without the sites and organization storage accounts, such as development and
the tests, turns off everything that uses them with one property:

| Property | Default | Meaning |
|---|---|---|
| `kinotic.systemApi.disableAzureStorage` | `false` | When true nothing is written to or deleted from the accounts: every published UI is marked ready at once and nothing serves it, a removed UI's record is deleted without a removal workload, and the deploy job has no SBOM task |
...
| Property | Default | Meaning |
|---|---|---|
| `retentionDays` | `30` | How long the record of a run that has ended is kept, with its logs, before the sweep deletes it |
