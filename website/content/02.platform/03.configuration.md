...
The default follows the provider, since it has to be a directory the vm-manager can create workload directories under with the authority it runs with: a `CLOUD_HYPERVISOR` node is a provisioned machine whose vm-manager runs as a service account, while a `BOXLITE` node is whatever machine a developer started one on, where `/var/lib` is root-owned.

| Environment variable | Meaning | Default |
|---|---|---|
| `KINOTIC_WORKLOAD_DATA_DIR` (vm-manager) | Base directory every workload volume mount must live under | `/var/lib/kinotic/workloads`, or `~/.kinotic/workloads` on a `BOXLITE` node |
