...
A node that loses its connection to the server — a server restart, a network partition — reconnects on its own and is `ONLINE` again with its next heartbeat. The orchestrator marks it `OFFLINE` after `heartbeatTimeoutSeconds` without one, whatever it reported last, and marks every workload whose run is still open on it — starting, running, or stopping — with the condition `NODE_UNREACHABLE` on the workload's `state.conditions`. Every such mark, and every status a workload's run passes through, is entered in the `kinotic_watch_event` data stream with what caused it, so a workload's history can be read after the fact. The record keeps the last status the node reported and its room on the node, because a VM on the far side of a partition is still running; the node's next report of the workload is applied whatever its timestamp says, clearing the condition. Deregistering an `OFFLINE` node is the operator's word that it is not coming back: its open runs are recorded `FAILED` with it. Only a heartbeat or a registration writes `lastSeen`; a status or allocation change leaves it as it is, so it never hides a heartbeat from the timeout. A call to the node's vm-manager that cannot be delivered is an earlier sign: the orchestrator checks the vm-manager's registration at once and, finding none, marks the node `UNREACHABLE`, so nothing is placed on it while the heartbeat timeout runs; its next heartbeat brings it back to `ONLINE`. A start or stop the vm-manager took and never answered leaves its workload marked `NODE_UNREACHABLE` the same way, since the node may have carried it out, and a microservice whose VM is in that state keeps `DEPLOYED` in the console with the reason beside it.
