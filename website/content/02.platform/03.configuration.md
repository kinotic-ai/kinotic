...
| `syncDiskSizeMb` | `4096` | Size cap on the sync VM's root filesystem, where the package manager keeps its cache, enforced the same way |
| `runtimeMemoryMb` | `1024` | Memory of each microservice's runtime VM |
| `runtimeDiskSizeMb` | `1024` | Size cap on each runtime VM's root filesystem; the checkout is a read-only mount, so the root holds only what the microservice writes |
