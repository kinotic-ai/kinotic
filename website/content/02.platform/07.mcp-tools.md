...
The input schema's property names are the service contract's Java parameter names (`orderId` above), retained in the class file at compile time — the same declaration the invocation binds against, so the published schema and the runtime binding cannot drift even when an implementation renames a parameter in its override. A single parameter can carry a different contract name with `@Name("...")`, declared on the contract.
