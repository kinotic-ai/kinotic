...
The issued token carries `aud=kinotic-mcp`, the audience `POST /mcp` serves. Its `sub` is the user who approved the consent, so the host acts as that user's `SystemParticipant`, `OrganizationParticipant`, or `ApplicationParticipant` and sees the same visibility matrix as every other caller. A Claude Code plugin needs nothing beyond the server URL:
...
**Static headers** — for agent frameworks and scripts that can attach headers: `clientId`/`clientSecret` (plus `organizationId`/`applicationId` to select a scope), or `Authorization: Bearer <kinotic-jwt>` minted for `kinotic-mcp` — see [System Security](/platform/system-security) for the audience table. When scope headers accompany a bearer token they must match the token's claims; a bearer-only request takes its scope from the token itself.
