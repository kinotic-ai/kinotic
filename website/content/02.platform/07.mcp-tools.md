...
- **On the service interface itself** — every function becomes a tool. A bare `@McpTool` on the interface exposes a whole service, including functions inherited from a CRUD parent, with per-function descriptions from each function's Javadoc (or its name). A type-level `title` or `description` applies to every function it sweeps in, so declare one only when it reads correctly for all of them; its hints are not applied at all (see [Hints](#hints)).
- **On an interface method** — that method becomes a tool, and it is that method's declaration (see [Which declaration describes a function](#which-declaration-describes-a-function)).
...
A function's `title`, `description`, and hints all come from **one** declaration — the nearest one to the function, chosen by whether it is there at all, never by what it says:
...
3. The type-level `@McpTool` that swept it in.

Only what that declaration leaves empty is derived, and only for the two strings: a blank `title` becomes the service-qualified name, and a blank `description` becomes the Javadoc then the function name. Nothing is borrowed from a declaration further down the list — `@McpTool(title = "Store Entity")` on a method whose inherited `@McpToolInfo` sets a `description` serves the Javadoc, not that `description`, because the method's own `@McpTool` is what describes it.

## Hints

The three hints (`readOnlyHint`, `destructiveHint`, `idempotentHint`) are served as MCP tool annotations. They follow the same single-declaration rule, with one difference: a boolean has no blank value to derive from, so a type-level `@McpTool` — which describes the service rather than any one function — does not supply them. A function it sweeps in is hinted by its own name instead.

That makes the hints exactly what the winning declaration says, including when it says nothing. `@McpTool(description = "Appends an audit entry")` on a method named `saveAuditEntry` serves `destructiveHint: false`: the annotation is present, so it decides, and it declares nothing destructive. The upside is that a type-level `readOnlyHint = true` can no longer mislabel the mutating functions a sweep catches.
