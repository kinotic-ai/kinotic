...
4. **The function name.** Every word counts, not just the leading verb, and the rules are tried in this order:
   - A verb that replaces or removes state (`save`, `update`, `put`, `set`, `upsert`, `delete`, `remove`, `clear`, `drop`, `purge`, `destroy`, `truncate`) is **destructive and idempotent** — it is not an additive update, and repeating it lands on the same state.
   - A name ending in `IfNotExist`/`IfNotExists` is **idempotent**.
   - A verb that adds or acts (`create`, `add`, `insert`, `register`, `send`, `run`, `start`, `stop`, `retry`, …) states **no hint** — and, reached before the reading rule, keeps a name like `getOrCreatePerson` from being served as read-only.
   - A reading verb (`find`, `get`, `list`, `search`, `count`, `query`, `read`, `fetch`, `is`, `has`, `exists`, `load`) anywhere in the name is **read-only**, so `peopleCount` reads as a count.
   - Any other name — `notifyPeople`, `retryRepoInitialization` — states no hint.

   Mutating rules are tried before the reading rule on purpose: serving a mutation as read-only invites a host to call it unattended, which is the one mistake worth ordering around.
