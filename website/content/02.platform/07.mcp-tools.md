...
The three hints (`readOnlyHint`, `destructiveHint`, `idempotentHint`) are served as MCP tool annotations. Exactly one declaration supplies all three — the nearest one to the function, chosen by whether it is there at all, never by what it says:
...
3. **The function name**, when the function carries neither.

Because the choice is by presence, a declaration that sets no hint is served with all three `false` rather than deferring to something else. `@McpTool(description = "Appends an audit entry")` on a method named `saveAuditEntry` serves `destructiveHint: false` — the annotation is what decides, and it says nothing is destructive. A type-level `@McpTool` is not in this list: it sweeps functions in without describing any one of them, so `readOnlyHint = true` there cannot mislabel the mutating functions the sweep catches.
