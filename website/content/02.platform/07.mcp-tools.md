...
**OAuth 2.1 (MCP authorization spec)** — what Claude connectors, Claude Code plugins, and other MCP hosts use. An unauthenticated request gets `401` with `WWW-Authenticate: Bearer resource_metadata="…/.well-known/oauth-protected-resource/mcp"`, from which a host discovers the authorization server (RFC 9728 + RFC 8414, both served from `kinotic.domain.apiBaseUrl`), registers itself as a public client (RFC 7591, `POST /api/auth/oauth/register`), and runs the PKCE S256 authorization-code flow: `GET /api/auth/oauth/authorize` sends the browser to the SPA's `/oauth/consent` page, where the signed-in user approves, and `POST /api/auth/oauth/token` (form-encoded) exchanges the single-use code for a one-hour access token plus a rotating 90-day refresh token (`offline_access` is advertised, so hosts refresh without re-consent; refresh-token reuse revokes the whole token family).

The issued token carries `aud=kinotic-mcp` and is accepted at `POST /mcp` and nowhere else — presenting it at the STOMP WebSocket handshake fails the audience check, so an MCP host cannot reach the published-service RPC surface with it. Its `sub` is the user who approved the consent, so the host acts as that user's `SystemParticipant`, `OrganizationParticipant`, or `ApplicationParticipant` and sees the same visibility matrix as every other caller. The refresh-token lineage is pinned to the same audience, so rotation never widens it. A Claude Code plugin needs nothing beyond the server URL:
...
**Static headers** — for agent frameworks and scripts that can attach headers: `clientId`/`clientSecret` (plus `organizationId`/`applicationId` to select a scope), or `Authorization: Bearer <kinotic-jwt>` where the JWT was minted for `kinotic-mcp`. When scope headers accompany a bearer token they must match the token's claims; a bearer-only request takes its scope from the token itself. The CLI's device-grant token is minted for `kinotic` and is rejected here — see [System Security](/platform/system-security) for the audience table.
