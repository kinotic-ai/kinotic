...
`kinotic.domain.oauth.allowedClientIds` lists the document URLs that may start a flow. A `client_id` that is not listed is rejected and an empty list permits none, so clients are onboarded explicitly — the deployment pattern of §6.10. A host that validates against the rules above still needs its document URL added before it can connect.
