...
A machine holds one of exactly two scopes: SYSTEM for platform daemons such as the vm-manager, or APPLICATION for an API client of one application — where it acts with that application's scope, exactly the position an application end-user occupies. Org-scope machines are rejected at the domain layer; nothing legitimately needs a machine wielding org-admin authority.
