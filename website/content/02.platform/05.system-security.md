...
Both grants are bound to the server that started them. The authorization request and the device-code flow record the OAuth issuer of the server whose endpoint began them, and only that issuer's token endpoint redeems the code: an authorization code presented to another server's token endpoint is refused and burned, and a device code polled there reads as unknown.
