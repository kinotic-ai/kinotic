...
For the Application scope, end-user provisioning is admin-driven: an org admin (or the application's own admin tooling) creates `IamUser` rows in the application's scope, or invites them — see [Organization Management](/platform/organization-management). There is no self-service sign-up into an application scope; an application's OIDC login authenticates users who already exist in that scope.
