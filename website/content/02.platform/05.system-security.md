...
Every `IamUser` carries its scope structurally, in two optional string fields — `organizationId` and `applicationId`. Which of them is set determines the layer: neither set → `SYSTEM`, `organizationId` only → `ORGANIZATION`, both set → `APPLICATION`.
...
| **System** | both null | Platform operators running kinotic-server itself | Out of scope for OIDC; handled via a separate, infrastructure-tier path (deferred) |
...
Authentication happens at the WebSocket upgrade. Every request carries optional scope headers (`organizationId`, `applicationId`) that determine which user pool and which OIDC configurations to check — presence of each id selects the layer. For OIDC initiated through the kinotic-server's own routes (`/api/login/start/:provider`), the scope is derived server-side from the matched `IamUser` and embedded in the minted JWT's claims; a non-browser client supplies the headers (or a JWT carrying the matching claims) on the upgrade itself.
...
The user entity stores `organizationId` and `applicationId` as plain string fields rather than typed references, and the layer is read structurally from which ids are set. This avoids coupling `IamUser` to `Organization`/`Application`, keeps queries simple (term filters on keyword fields), and lets the scope model evolve without migrating user records.
...
### JWT `aud` names the one surface the token may be used on

Every Kinotic-minted JWT carries an `aud` claim naming the entry point it was issued for, and each entry point accepts only its own audience:

| `aud` | `KinoticAudience` | Accepted at | Issued by |
|---|---|---|---|
| `kinotic` | `PUBLISHED_SERVICES` | the STOMP WebSocket handshake | the device-code grant (the CLI) |
| `kinotic-mcp` | `MCP_TOOLS` | `POST /mcp` | the authorization-code grant (MCP hosts) |

An MCP host's token therefore cannot open a STOMP connection to the published-service RPC surface, and the CLI's token cannot call MCP tools. The audience also rejects any non-Kinotic JWT — even a valid IdP token — since it carries neither value.

The audience is chosen by the endpoint being called, never read from the request, and a refresh-token lineage is pinned to the audience its grant was redeemed for, so rotation can never cross surfaces.
...
`kinotic.domain.oauth.allowedClientIds` restricts which document URLs may start a flow. Left empty, any URL that validates is accepted, which is what lets an MCP host connect with no prior arrangement. Populated, only the listed clients can — the enterprise pattern of §6.10, for deployments that want to onboard named clients explicitly.
