...
### JWT `aud` records the surface a token was issued for

Every Kinotic-minted JWT carries an `aud` claim naming the entry point its grant serves:
...
A refresh-token lineage records the audience its grant was redeemed for, so rotation stamps replacements with the same one.

::alert{type="info"}
**No entry point verifies the claim today.** A token is accepted at every Kinotic surface whatever its `aud` says, so an MCP host's token can open a STOMP connection and the CLI's token can call MCP tools. What the bearer may do once it reaches a surface is still bounded by the `sub` claim's participant scope, which every entry point resolves the same way.
::
