...
### A Kinotic-minted JWT is accepted at every Kinotic entry point

A Kinotic-minted JWT carries no `aud` claim, and no entry point restricts which grant a token came from. A token minted by the device-code grant for the CLI and one minted by the authorization-code grant for an MCP host are interchangeable: either opens a STOMP WebSocket handshake and either calls `POST /mcp`. What the bearer may do at whichever surface it reaches is bounded by the `sub` claim's participant scope, which every entry point resolves the same way.

A refresh-token lineage is likewise unrestricted — rotation re-mints a token with the same reach as the one it replaces.
