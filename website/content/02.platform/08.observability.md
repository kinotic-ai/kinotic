...
Every node runs a managed [Grafana Alloy](https://grafana.com/docs/alloy/latest/) process whose pipeline is regenerated as workloads come and go; Alloy tails each running workload's log files and pushes them to Loki, and receives the traces of the workloads that elect tracing and pushes them to Tempo (see [Workload Traces](#workload-traces)). What a workload has to do for its logs to be shipped depends on the node's [VM provider](/platform/configuration#vm-provider):
...
A workload that sets `tracing` to `true` is given an OTLP endpoint of its own on the node it runs on, and the spans it exports there are shipped to Tempo under its organization's tenant, carrying the same identity as its log streams. Tracing is elected per workload: the node ships nothing for a workload that leaves it off, whatever the workload's runtime exports.
...
| `OTEL_METRICS_EXPORTER`, `OTEL_LOGS_EXPORTER` | `none` — the endpoint carries traces and nothing else |

These are laid over the workload's own `environment` and `secrets`, so a workload cannot point its exporter elsewhere by setting them itself. The endpoint holds for the workload's life, restarts included, and is released when the workload is destroyed. Each receiver accepts only its own token, so a guest cannot push into another workload's stream. A workload electing tracing on a node that ships no traces (no `KINOTIC_TEMPO_URL`) runs without an endpoint; the node logs the election it could not honour.

### What the workload must do

Export traces over OTLP from that environment. An OpenTelemetry SDK configured from the environment — the Java agent, or the Node SDK with its default configuration — does so without any code of its own. The Kinotic runtimes instrument every service invocation through the OpenTelemetry API, so a Kinotic application whose process runs such an SDK ships a span per call, continuing the trace of the caller that invoked it. A runtime that ignores the variables ships nothing.

A workload with `network.mode` `DISABLED` has no way to reach the endpoint, so a node refuses one that also elects tracing rather than starting it silent.
...
| `BOXLITE` | boxlite's host alias `192.168.127.254`, whose proxy completes the connection over the host's loopback | Binds the receiver to loopback and adds the alias to the workload's egress allowlist. boxlite's allowlist matches hosts rather than ports, so a traced workload on a `BOXLITE` node can reach every loopback service of the node, not only its receiver |

The port is opened by the same per-workload rules as the workload's [egress](/platform/configuration#workload-egress), so a `CLOUD_HYPERVISOR` node that does not deny egress by default refuses a workload electing tracing, as it refuses one declaring `allowedHosts`.

### Identity and tenant

Every span arrives with these resource attributes, matching the labels on the workload's log streams:
...
Spans are pushed with `X-Scope-OrgID` set to the organization id, or `kinotic-system` for platform workloads with no organization — the same tenant its logs go to. A Tempo running multi-tenant (`multitenancy_enabled: true`) keeps each organization's traces apart the way Loki does; a single-tenant Tempo, such as the one in the docker-compose stack, ignores the header and stores everything together.

## Configuration

| Setting | Description |
|---|---|
| `kinotic.managementApi.loki.url` / `KINOTIC_MANAGEMENTAPI_LOKI_URL` (server) | Loki HTTP API the server queries (default `http://localhost:3100`) |
