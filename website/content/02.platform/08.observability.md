...
Every micro VM the platform runs for an organization is a workload of that organization: the long-lived VM of each project microservice, and the one-off runs that sync its projects and publish or remove their UIs. A workload whose run has ended keeps its record, with the status and exit code the run ended on.

The `WorkloadMonitoringService` (`@kinotic-ai/management-api`, `Kinotic.workloadMonitoring`) reads the workloads of the caller's organization: `findWorkloads` pages through all of them, `findWorkloadsForApplication` through one application's, and `findWorkload` returns one, failing alike for a missing workload, another organization's, and the platform's own. It admits organization participants only; a platform operator reads every workload through the system zone's `WorkloadService` instead. A record holds a workload's environment variables and the names of its secrets, never their values.

The portal lists them under **Runtime → Workloads** in the organization, application, and project sidebars, with the count in each state and, across the organization, a filter by application. A workload's page shows its status, exit code, owner, resources and run time, what its record holds, its logs, and — for a workload that elects telemetry — its traces and metrics on the **Telemetry** tab. A workload a project's deployment started leads to that project's **Deployment** page, where a microservice is restarted or removed. The system console lists the same workloads, and every other, on its **Workloads** pages, where an operator can also stop and destroy one.
...
The portal shows them on an application's **Observability** page: a trace search — by service, span name, errors, and duration — opening each trace on its own page as a waterfall of its spans with their attributes, and the requests, errors, and latency of the application's services, the busiest calls over the range with the share of each that failed and its 95th percentile duration, and a free PromQL query, the errors chart leading to the traces with a failed span. The organization's **Observability** page has the same view across all of its applications or one of them, and a workload's **Telemetry** tab the same view narrowed to that workload's `workload_id`. The system console has them at **Organizations → (organization) → Observability**, with each application's view under **Applications → (application) → Observability** and each workload's on its page. The console's own **Observability** page, under Platform, reads the system tenant: the traces and metrics of the servers, the gateway, and the worker nodes themselves.

The overview pages show, under **Traffic · last hour**, the invocations clients made through the gateway: the calls per second, the share that failed, and the 95th percentile time to first reply, each with its peak and its course over the hour — for the organization on its **Overview**, summed over its applications, for an application on its overview, and, in the system console, for each organization and application and for the whole platform on the **Dashboard**.

The gateway records every invocation that awaits a reply in the `kinotic.gateway.invocation.duration` histogram, in seconds (`kinotic_gateway_invocation_duration_seconds` in Mimir), attributed to the `organization_id` and `application_id` its caller acts for and to its `outcome`: `ok`, `error` when the service replied with an error, `unavailable` when no service took it or the node serving it left before replying, or `cancelled` when the client cancelled it or closed its connection. A stream counts once, when it ends, timed to its first value. The server's OpenTelemetry agent exports the histogram to the platform tenant, so `TelemetryService.queryTraffic` builds its expression on the server: the caller names a signal — `REQUESTS`, `ERRORS`, or `LATENCY_P95` — an organization, and optionally one of its applications, and an organization participant may name only its own. The span metrics stay on the **Observability** pages, broken down by service and call.

The system console's **Cluster** page charts the servers' own runtime, which the kinotic-server's OpenTelemetry agent exports to the system tenant under `job="kinotic-server"`: each server process's CPU, heap and non-heap memory against the heap limit, time spent in garbage collection by collector, live and blocked threads, direct buffer memory by pool, and the HTTP requests the servers answer with the share that failed with a 5xx and their 95th percentile duration. The **Dashboard** carries the busiest server's CPU and the share of the servers' heap in use under **Servers · last hour**, beside the platform's traffic.
