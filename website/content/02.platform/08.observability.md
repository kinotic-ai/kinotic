...
The overview pages show the same span metrics summed over the last hour under **Traffic · last hour**: the calls per second, the share that failed, and the 95th percentile duration, each with its peak and its course over the hour — for the organization on its **Overview**, for an application on its overview, and, in the system console, for each organization and application and for the platform itself on the **Dashboard**.
