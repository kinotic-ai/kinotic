...
- **Id** -- A unique identifier made of lowercase letters, digits, and interior dashes or underscores (e.g., `inventory-app`). The id forms the final label of the application's [zone](/reference/cri-format), so an id given in any other form is normalized to this shape at creation.
