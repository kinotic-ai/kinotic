...
A getter or method decorator that marks the member providing the scope identifier. Scope narrows which service instance handles a request, which is useful for multi-tenant or per-device services.
...
### @Context(...parameterIndices)

A method decorator that marks which parameters receive the request context. The context carries information about the caller, such as the authenticated participant.
