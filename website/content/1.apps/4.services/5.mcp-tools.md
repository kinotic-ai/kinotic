...
The tool name is always minted from the service's qualified name and the function, encoded to satisfy the MCP tool-name pattern: dots become underscores and the function is appended after a dash.
...
Because a qualified name is unique system wide, so is the minted tool name — and a customer service's tool name always carries its organization and application ids via the zone prefix, as above. A qualified name deep enough to overflow the 128-character MCP limit fails registration rather than being truncated.
