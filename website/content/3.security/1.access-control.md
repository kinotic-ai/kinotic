...
| Placement | Enforcement Point | Engine |
|-----------|------------------|--------|
| Service method | Gateway (before invocation) | Cedar (in-process via JNI) |
| Entity decorator | Persistence layer (query filter) | Elasticsearch query compiler |
...
External policy engines move policy definitions to YAML files or scripts in a separate repository. This creates **indirect coupling** — policy files that reference field names from your data model, convention-based links between service names and policy resource kinds, and a separate deployment pipeline that must stay in sync with code changes. Developers context-switch between their code and a policy repo to understand what is actually enforced.
...
Apply `@AbacPolicy` to published service methods to enforce authorization at the gateway before the call reaches the service. The gateway evaluates the policy against the method's arguments and the caller's identity.

### TypeScript

```typescript
import { AbacPolicy } from '@kinotic/core'
...
```java
import org.kinotic.auth.api.annotations.AbacPolicy;
...
public class OrderService {
    ...
    @AbacPolicy("participant.roles contains 'finance'")
    @AbacPolicy("transfer.amount <= participant.transferLimit")
    public void transferFunds(Transfer transfer, Approval approval) {
        // ...
    }
}
```
...
1. The service registers with the gateway and declares its methods and parameter names
2. The gateway parses each `@AbacPolicy` expression into an AST at registration time
3. The AST is compiled into a Cedar policy with parameter names mapped to resource attributes
4. When a request arrives, the gateway transforms the raw JSON argument array into a named object using the registered parameter names
5. Cedar evaluates the policy in-process — no network hop, no serialization overhead
6. If the policy denies the request, the gateway rejects it before it ever reaches the service
...
The raw JSON payload arrives as an array (`[{"amount": 50000}, {"approved": true}]`), and the gateway transforms it to a named object (`{"transfer": {"amount": 50000}, "approval": {"approved": true}}`) using parameter names registered at startup.

## Entity Policies

Apply `@AbacPolicy` to entities via `@EntityServiceDecorators` to enforce authorization at the persistence layer. Policies are compiled into Elasticsearch query filters, so unauthorized records are never returned from the data store.

### TypeScript

```typescript
import { Entity, AutoGeneratedId } from '@kinotic/persistence'
import { EntityServiceDecorators, $AbacPolicy } from '@kinotic/persistence'
...
Entity policies can target specific operations or operation groups:
...
You can also target individual operations for fine-grained control:
...
1. When an entity is registered, the platform parses each `@AbacPolicy` expression into an AST
2. At query time, the AST is compiled into an Elasticsearch query filter using `EsQueryCompiler`
3. `participant` attributes are resolved to concrete values from the caller's security context
4. The compiled filter is injected into the Elasticsearch query, restricting results at the data layer
5. For mutations (`save`, `update`, `delete`), the policy is evaluated against the incoming entity data
...
The policy `entity.sharedWith contains participant.id` never changes. When a user shares a photo, the application updates the photo's `sharedWith` array in the database. The next time anyone queries photos, the existing policy filters results to only include photos shared with the caller.

```typescript
// The service method handles the sharing logic
class PhotoService {
    ...
    async getMyPhotos(): Promise<Photo[]> {
        return await this.photoEntityService.findAll()
    }
}
```

When `getMyPhotos()` is called, the persistence layer automatically injects the filter so only photos where `sharedWith` contains the caller's ID are returned.

## Combining Both Layers

Service method policies and entity policies work together. A typical application uses both:

```typescript
// Gateway enforcement — can this caller use this service at all?
class PhotoService {
...
In this example:
- The gateway ensures only authenticated users with the `'user'` role can call `sharePhoto`
...
ABAC policies can express pure role-based checks. Use `participant.roles contains` to gate access by role:
...
This means RBAC is a subset of ABAC — no separate role-checking mechanism is needed.
