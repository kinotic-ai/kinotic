...
CREATE DATA STREAM <stream_name> (<column_name> <type> [, <column_name> <type>]*)
    [WITH (LIFECYCLE = '<policy_name>')] ;
```

- `LIFECYCLE` (optional): the name of a `CREATE LIFECYCLE POLICY` that ages data out via ILM. Define the policy before the stream that references it.
- `<type>`: See [Supported Types](#supported-types).
...
Creates a reusable [ILM](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-lifecycle-management.html) policy that data streams reference by name to age old data out. A policy is shared: many data streams can point at the same one.

**Syntax:**
```sql
CREATE LIFECYCLE POLICY <policy_name> WITH (<phase> [, <phase>]*) ;
```
...
|-------|------|-------------|
| `ROLLOVER` | `ROLLOVER (<condition> [, <condition>]*)` | Hot-phase rollover of the write index when any condition is met |
| `DELETE` | `DELETE (MIN_AGE = '<age>')` | Deletes a backing index once it is older than `MIN_AGE` (measured from rollover) |

**Rollover conditions:**

| Condition | Allowed Values | Description |
|-----------|---------------|-------------|
| `MAX_AGE` | String (e.g., `'7d'`) | Roll over when the write index reaches this age |
| `MAX_SIZE` | String (e.g., `'50gb'`) | Roll over when the write index reaches this total size |
| `MAX_DOCS` | Integer (e.g., `1000000`) | Roll over when the write index reaches this document count |
...
`ABORT`, `ADD`, `ALTER`, `AND`, `AUTO`, `BINARY`, `BOOLEAN`, `COLUMN`, `COMPONENT`, `CONFLICTS`, `CREATE`, `DATA`, `DATE`, `DECIMAL`, `DELETE`, `DOUBLE`, `EXISTS`, `FLOAT`, `FOR`, `FROM`, `GEO_POINT`, `GEO_SHAPE`, `IF`, `INDEX`, `INDEXED`, `INSERT`, `INTEGER`, `INTO`, `JSON`, `KEYWORD`, `LIFECYCLE`, `LONG`, `MAX_AGE`, `MAX_DOCS`, `MAX_SIZE`, `MIN_AGE`, `NESTED`, `NOT`, `NUMBER_OF_REPLICAS`, `NUMBER_OF_SHARDS`, `OBJECT`, `OR`, `POLICY`, `PROCEED`, `QUERY`, `REFRESH`, `REINDEX`, `ROLLOVER`, `SCRIPT`, `SET`, `SIZE`, `SLICES`, `SOURCE_FIELDS`, `STREAM`, `TABLE`, `TEMPLATE`, `TEXT`, `TRUE`, `FALSE`, `UNION`, `UPDATE`, `USING`, `UUID`, `VALUES`, `WAIT`, `WHERE`, `WITH`, `SKIP_IF_NO_SOURCE`
