...
This matters because every direct `GET /<index>/_doc/<id>` lookup — including `findById` on `CrudServiceTemplate`, `AbstractCrudService`, and `IamCredentialStore` — resolves the document by `_id`. A row inserted without an `id` column cannot be retrieved by its logical identifier later, only found via search.

As a rule: if the target index's model has a logical `id` field (which all `Identifiable<String>` entities do), always include the `id` column in the INSERT so `_id` stays in sync with the document's `id` field.

```sql
-- Recommended: id column promoted to _id
INSERT INTO kinotic_iam_user (id, email, authScopeType, authScopeId, enabled)
    VALUES ('00000000-0000-0000-0000-000000000001', 'admin@example.com', 'SYSTEM', 'kinotic', true)
    WITH REFRESH ;

-- Avoid for entity tables: no id column means a random _id will be generated
INSERT INTO audit_log (event, timestamp) VALUES ('startup', '2024-01-01') ;
```
