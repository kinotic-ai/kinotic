...
CREATE DATA STREAM <stream_name> (<column_name> <type> [, <column_name> <type>]*)
    [WITH (DATA_RETENTION = '<period>')] ;
```

- `DATA_RETENTION` (optional): a [data stream lifecycle](https://www.elastic.co/docs/manage-data/lifecycle/data-stream) retention period (e.g. `'30d'`). Elasticsearch rolls the backing indices over automatically and deletes data older than this. This is the native data stream lifecycle (`template.lifecycle.data_retention`) — not ILM, so there is no separate policy object or bootstrap index to manage.
- `<type>`: See [Supported Types](#supported-types).
