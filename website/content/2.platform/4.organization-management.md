...
10. Backend returns a Kinotic JWT (60s TTL ticket).
    ...
    completes the REST login, establishing the session cookie used to authenticate
...
When `lookup` returns `{type: "password"}`, the frontend collects the password and exchanges email + password for a Kinotic JWT — **the same JWT shape the OIDC paths produce** — so the SPA only ever holds a Bearer-issued session, never the raw password on the wire:
...
2. LoginHandler.handleToken:
   ...
   - on success: mints the 60s Kinotic JWT (organizationId + applicationId + aud=kinotic)
                 returns { "token": "<jwt>" }
   - on any failure: 401 "Invalid credentials"
                     (deliberately generic — covers unknown email, wrong password,
                      OIDC user, disabled user)
3. Frontend completes the REST login with the JWT, establishing the session cookie
   that authenticates the WebSocket upgrade
```

The frontend never sends raw passwords over the WebSocket. Non-UI clients (CLI, automation) authenticate directly at the upgrade instead — a Node client sets `ConnectionInfo.webSocketFactory` to send `clientId` / `clientSecret` plus the `organizationId` / `applicationId` scope headers; the SPA does not use that path.
...
4. OidcLoginHandler.handleCallback:
   ...
   - if one exists (or multiple — picks the one with primary=true):
       mint a 60s Kinotic JWT carrying organizationId+applicationId+aud=kinotic
       302 to <loginSuccessPath>#token=<jwt>
5. App.vue lifts the JWT from the fragment and completes the REST login,
...
For the Bearer path the kinotic-server validates the JWT signature against its rotated signing keys, asserts `aud=kinotic`, and creates the `Session`. The JWT TTL is 60s — long enough to open the WebSocket once, not long enough to be useful if leaked.
...
| `POST` | `/api/login/token` | `LoginHandler` | Email + password → Kinotic JWT (`{token}`). UI path; non-UI clients authenticate directly at the WebSocket upgrade with `clientId` / `clientSecret` instead |
