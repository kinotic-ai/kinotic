...
- **Tool names are always minted, never parsed.** A name is derived from the service and function names, no override exists, and nothing ever parses a name apart to make a decision — resolution is a caller-scoped directory query and authorization reads the zone from the stored CRI, so the name is never a trust input.
...
- **Ambiguous names are refused, not resolved.** If a name is provided by more than one service visible to the caller, the platform never picks a winner: the providing CRIs are logged for operators and the caller receives a generic internal error.
