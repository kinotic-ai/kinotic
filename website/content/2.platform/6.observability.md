...
Workload VMs run detached from the vm-manager process, and the vm-manager persists each workload's state on the node. If the vm-manager restarts (a crash, a systemd restart), it reattaches to the VMs that are still running and regenerates the Alloy pipeline, so their logs keep shipping.
