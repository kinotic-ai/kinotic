...
The tool name is always minted from the service's simple name and the function:

```
srv://os-api~com.acme.OrderService/findById  →  OrderService.findById
```

Names are never parsed back apart — resolution always queries with the caller's (organization, application) scope, and authorization reads the zone from the tool's stored CRI, not from the name. A name therefore only has to be unique among the tools one scope can see: services in different scopes share names freely, and if two services visible to the same scope mint the same name, a call to it is refused as ambiguous rather than resolved to either. A name too long for the 128-character MCP limit fails registration rather than being truncated.
