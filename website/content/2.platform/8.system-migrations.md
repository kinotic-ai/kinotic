...
If two variants declare the same field name with different types — e.g. `title TEXT` in one variant and `title KEYWORD` in another — the migration executor will throw an `IllegalArgumentException` at mapping time. Fields shared across variants must have identical types.
