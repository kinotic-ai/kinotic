...
Kinotic-minted JWTs (the 60s tickets passed from OIDC callback to the WebSocket upgrade) include an `aud` claim of `kinotic` so that any non-Kinotic JWT — even a valid IdP token — is rejected at the gateway.
