...
Since omitting a sub-field keeps it, `null` is what clears one — at any depth, or on a whole column:

```sql
UPDATE persons SET address = { street: null } WHERE id == 'p-1' WITH REFRESH;
UPDATE persons SET nickname = null WHERE id == 'p-1' WITH REFRESH;
```
