...
A composite column is set from the same literals INSERT uses. `SET` replaces the column's whole
value rather than merging into it, so write every sub-field you want to keep:

```sql
UPDATE persons SET address = { street: '99 Elm St', city: 'Shelbyville' } WHERE id == 'p-1' WITH REFRESH;
```
