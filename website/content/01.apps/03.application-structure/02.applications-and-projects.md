...
| --- | --- | --- |
| `KINOTIC_UI_BASE_PATH` | `/<commit sha>/` | The path the built assets are served under. Assets are addressed by commit and cached for a year, so a new commit is always a new path |
