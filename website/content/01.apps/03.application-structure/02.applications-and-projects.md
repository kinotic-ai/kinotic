...
- **Id** -- Derived from the name at creation: the slugified name, made of lowercase letters, digits, and single interior dashes (e.g., `inventory-app`). The id forms the final label of the application's [zone](/platform/reference/cri-format), and joined to the organization id with `--` it forms the application's host label, `<organizationId>--<applicationId>`, which DNS limits to 63 characters: a name whose label would be longer is rejected.
- **Description** -- A human-readable summary of what the application does.
- **Primary UI** -- The name of the UI whose site the application's browser flows, such as OAuth consent, return to, chosen on the application's Settings page. It must name a UI one of the application's projects has published; it stays set if that UI's deployment is later removed.
...
| --- | --- | --- |
| `VITE_KINOTIC_HOST` | e.g. `acme--orders.apps-api.kinotic.ai` | The host the UI connects to Kinotic on from a browser |
