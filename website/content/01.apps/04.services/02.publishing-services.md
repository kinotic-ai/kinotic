...
import { isOrganizationParticipant } from '@kinotic-ai/management-api'

@Publish('com.example')
class OrderService {
    @Context
    async listOrders(status: string, context: ServiceContext): Promise<Order[]> {
        const caller = context.participant
        if (!caller || !isOrganizationParticipant(caller)) {
...
`context.participant` is the participant the gateway authenticated on the calling connection, serialized with its scope discriminator and ids (`type`, `organizationId`, `applicationId`); the guards in `@kinotic-ai/management-api` narrow it to the scope-typed participant interfaces. It is absent only for an invocation that originated inside the platform with no participant bound, such as a scheduled job.
