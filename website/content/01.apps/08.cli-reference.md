...
Each entity's C3Type schema is written as JSON to `.config/c3/entities/<namespace>.<name>.json`.
...
Synchronize local entity definitions with the Kinotic server. This uploads your entity classes so the server can set up the backing data stores and register the entity services. It runs the same generation as `kinotic generate`, so `.config/c3/entities` is refreshed as well.
