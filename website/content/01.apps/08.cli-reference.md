---
title: CLI Reference
description: Command reference for the Kinotic CLI.
navigation:
  icon: i-lucide-terminal
---

## Installation

```bash
bun install -g @kinotic-ai/kinotic-cli
```

## Commands

### `kinotic login`

Log in to a Kinotic server and store credentials for subsequent commands. Runs the OAuth device-authorization flow: the CLI opens a browser once for you to approve, then stores a refresh token so later commands are non-interactive. The server you log in to becomes the default server for commands like `kinotic sync`.

```bash
kinotic login --server http://localhost:58503
kinotic login
```

**Flags:**

| Flag | Description |
|------|-------------|
| `-s, --server` | The Kinotic server URL to log in to. When omitted, the stored default server is used |

---

### `kinotic create project`

Create a new Kinotic project scaffolded as a Bun workspace mono repo, with `packages/domain` (entity model and generated repositories), `packages/microservices`, `packages/ui`, and a `.config/kinotic.config.ts` wired to the given organization and application. After generation, the CLI interactively offers to add a Library or Frontend package.

```bash
kinotic create project MyProjectName --organization acme --application acme-app
kinotic create project MyProjectName -o acme -a acme-app
```

**Arguments:**

| Argument | Description |
|------|-------------|
| `name` | The name for the project (also the directory created) |

**Flags:**

| Flag | Description |
|------|-------------|
| `-o, --organization` | **Required.** The organization that owns the project |
| `-a, --application` | **Required.** The application the project belongs to |

---

### `kinotic init` / `kinotic initialize`

Initialize an existing directory as a Kinotic project. Creates the `.config/kinotic.config.ts` configuration file pointing at your entity definitions and generated-repository directories. Both directories must already exist. Flags that are omitted are prompted for interactively.

```bash
mkdir -p src/model src/repository
kinotic init --application my.app --entities src/model --repository src/repository
```

**Flags:**

| Flag | Description |
|------|-------------|
| `-a, --application` | The application name (must start with a letter; letters, numbers, periods, underscores, or dashes) |
| `-e, --entities` | Path to entity definitions directory |
| `-r, --repository` | Path for generated repository classes |
| `-m, --mirror` | Mirror the entity folder structure under the repository path (default: true) |

---

### `kinotic generate` / `kinotic gen`

Generate Repository classes from your entity definitions. This runs locally without connecting to the server, and fills in the implementations of [named query](/apps/persistence/named-queries) methods declared with `@Query`.

```bash
kinotic generate
kinotic gen -v
kinotic gen --force
```

**Flags:**

| Flag | Description |
|------|-------------|
| `-v, --verbose` | Enable verbose logging |
| `-f, --force` | Force full regeneration, ignoring incremental change detection |

---

### `kinotic sync` / `kinotic synchronize`

Synchronize local entity definitions with the Kinotic server. This uploads your entity classes so the server can set up the backing data stores and register the entity services, regenerates the repository classes, and applies any pending [migrations](/apps/persistence/migrations) from the `./migrations` directory. Requires a prior `kinotic login` against the target server.

```bash
kinotic sync -p
kinotic sync -p -v -s http://localhost:58503
```

**Flags:**

| Flag | Description |
|------|-------------|
| `-s, --server` | The Kinotic server URL. When omitted, the stored default server is used |
| `-p, --publish` | Publish each entity after save/update |
| `-v, --verbose` | Enable verbose logging |
| `--dryRun` | Enables verbose logging and does not save any changes to the server |
| `--force` | Force full regeneration, ignoring incremental change detection |

---

Run `kinotic --help` or `kinotic <command> --help` for the full list of commands and options.
