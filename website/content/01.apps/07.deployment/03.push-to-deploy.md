...
4. **Ensure organization storage** — when the commit contains a UI, makes sure your
   organization's storage exists: the first such deployment of an organization creates its
   storage account, which takes a few minutes, and later ones find it at once. A commit
   without UIs skips this step.
5. **Ensure runtime workloads** — one long-lived VM per microservice the commit contains,
...
6. **Publish UIs** — uploads every UI the build VM built to your organization's storage
   through a short-lived publish VM that holds nothing but an upload URL valid for the run,
   then records each UI's site. A UI's first publish mints its site's hostname and creates
   the site; later publishes keep the site and switch it to the new commit, retaining the
   previous commit's assets so tabs still open on it keep working. A UI the commit no longer
