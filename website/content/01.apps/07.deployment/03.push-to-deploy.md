...
2. **Sync project source** — a short-lived, sandboxed build VM brings the checkout to the
   ...
   not build never reaches your running services. Your entity definitions are then
   synchronized to the server, authenticated by a machine identity the platform
