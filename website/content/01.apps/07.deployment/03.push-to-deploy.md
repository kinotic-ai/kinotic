...
Every VM a deployment starts — the build VM, the publish VM, and each microservice's — is one
of your organization's [workloads](/platform/observability#workloads). The project's
**Workloads** page lists them, with the state each is in, the exit code of a run that ended,
its resources and its log; the same list sits under the application and across the
organization.
