...
6. **Generate SBOM** — keeps the project's [SBOM](#sbom) current. When the project has no SBOM
   of the dependencies the build VM installed, a short-lived SBOM VM generates it from the
   checkout, mounted read-only, uploads it over the project's SBOM file in your organization's
   storage through a URL valid for that file alone, and records it; otherwise no VM runs and the
   SBOM stays. The step expands on the job run page into the SBOM VM's log, or says the
   dependencies are unchanged. A checkout without a `bun.lock` fails the step.
...
`bun.lock` together with the generator's version and reports the hash with the commit's
artifacts; a report whose hash differs from the previous one drops the project's SBOM, and the
last step of the deployment generates it again. A push that changes code but no dependency keeps
the SBOM.

The project has one SBOM document, kept in your organization's storage and replaced whenever it
is generated. The project's **SBOM** page shows it for the last synced commit, lists its
components, searchable by name and license and filtered by how the project uses them, and
downloads the document as it is. The page reads the document through a URL the platform issues
for fifteen minutes once it has authorized you; `ProjectService.findDeployment`, whose
`sbomGenerated` says whether the document lists the current dependencies, and
`ProjectService.findSbomDocumentUrl` serve the same to any client of your organization, MCP hosts
included.
...
`FAILED` with the reason, and the project has no SBOM until a later deployment generates it,
since the dependencies it would list changed. An environment without
