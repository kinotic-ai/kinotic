...
the microservice's log, restart it, or remove the deployment. Each deployment carries what it
...
run ends is replaced, after a short wait when it failed, so a crashing service restarts at the
platform's pace rather than the node's; a VM on a node the platform cannot reach is left alone,
...
and what it is in `state.observed`; while the two differ, `failureMessage` says what the site
...
list the published UIs with their sites as well.
