...
the microservice's log, restart it, or remove the deployment; the same listing, restart and
removal are MCP tools, so an assistant connected to the platform sees each microservice's phase,
its message and a restart it is waiting for. Each deployment carries what it
...
list the published UIs with their sites as well. The listing and the removal are MCP tools too,
so an assistant connected to the platform sees each site's phase and what it last answered.
...
change to either is entered in the `kinotic_watch_event` ledger with what caused it. The project's
