...
6. **Generate SBOM** — keeps the project's [SBOM](#sbom) current. When the hash of the
   ...
   mounted read-only, and uploads it to your organization's storage through a URL valid for that
   project's directory alone, its only credential; Kinotic records the SBOM once the VM has
   succeeded. When the hash matches, no VM runs and the SBOM stays. The step expands on the job run page into the SBOM VM's log, or says the
