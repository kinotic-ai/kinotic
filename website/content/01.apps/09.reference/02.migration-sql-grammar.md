...
`SET` replaces the column's whole value rather than merging into it — sub-fields left out of the literal are removed from the stored document. Write the complete object every time:

```sql
-- address becomes { city: 'Shelbyville' }; the stored street is dropped
UPDATE persons SET address = { city: 'Shelbyville' } WHERE id == 'p-1' ;
```

A sub-field cannot be targeted on its own: `SET` takes a column name, not a dot-path, so `address.city` is not valid on the left of an assignment.
