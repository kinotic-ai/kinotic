...
Object literals are used by `INSERT` (see [Inserting composite columns](#inserting-composite-columns)). `UPDATE ... SET` and `WHERE` take scalar values only.
...
- **Literals:** `'string'`, `123`, `12.5`, `-3`, `true`, `false`
- **Parameters:** `?`
- **Binary Expressions:** `<field> + <value>`, `<field> - <value>`, etc.

Object and array literals are not expressions — an `OBJECT` or `NESTED` column is populated by `INSERT`, not by `UPDATE ... SET`.
