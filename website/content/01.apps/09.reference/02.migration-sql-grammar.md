...
- `<value>`: See [Values](#values) — a literal, a parameter (`?`), an object literal, or an array literal. A `null` stores no field at all, leaving the document the same as one whose column list never named it.
...
Because omitting a sub-field keeps it, `null` is what removes one. It works at any depth, and on a whole column:

```sql
-- street is removed from the stored address; city and the rest are untouched
UPDATE persons SET address = { street: null } WHERE id == 'p-1' ;

-- the whole address column is removed
...
A sub-field cannot be targeted on its own: `SET` takes a column name, not a dot-path, so `address.city` is not valid on the left of an assignment. Name the column and let the merge apply the sub-fields you list.
