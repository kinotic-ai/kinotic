...
| Array | `['ADMIN', 'USER']`, `[ { label: 'a' } ]` | A list of any of the forms above; the value form for a `NESTED` column or an `OBJECT` column holding a list |
...
| `OBJECT (...)` | Embedded object with declared sub-fields, one or a list of them; schema is fixed |
| `OBJECT (...) NOT INDEXED` | Embedded object; stored but sub-fields are not searchable |
| `NESTED (...)` | List of embedded objects, each indexed as its own document; only for a query that must match two sub-fields of the same element (see [NESTED](#nested)) |
