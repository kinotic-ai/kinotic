...
- `<value>`: See [Values](#values) — a literal, a parameter (`?`), an object literal, or an array literal.
...
- `<expression>`: See [Expressions](#expressions) — a value (including an object or array literal), a parameter, or a binary expression (e.g., `age + 1`).
...
The stored side decides too: if the column currently holds a scalar, an array, or nothing at all, there is nothing to merge into and the object is written whole.

::callout{type="info"}
**Merging means a sub-field cannot be removed by omitting it.** Since sub-fields left out of the literal survive, there is no way to clear one through `SET`. Delete and re-insert the document when a stored sub-field has to go.
::

A sub-field also cannot be targeted on its own: `SET` takes a column name, not a dot-path, so `address.city` is not valid on the left of an assignment. Name the column and let the merge apply the sub-fields you list.
...
**Values:** String, number, or boolean literal, or parameter (`?`). Object and array literals cannot be compared.
