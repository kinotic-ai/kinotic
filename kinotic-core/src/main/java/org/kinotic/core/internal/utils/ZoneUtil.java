...
public final class ZoneUtil {
    ...
    // DNS label rule, so a zone can never contain characters that would break CRI parsing,
    // dot-boundary prefix matching, or wildcard patterns. A zone is dot separated labels —
    // both patterns are views of the one grammar.
