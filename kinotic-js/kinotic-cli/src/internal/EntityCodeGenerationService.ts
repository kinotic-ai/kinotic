...
export class EntityCodeGenerationService {
    ...
    private async generateRepository(adminService: boolean,
        ...
        if (!fs.existsSync(entityServicePath)) {
            ...
            namedQueries = await this.processNamedQueries(adminService,
                                                          `${entityName}${adminPrefix}Repository`,
                                                          entityServicePath)
        }
     ...
     */
    private async processNamedQueries(adminService: boolean,
            ...
            if(serviceClass){
                ...
                for(const method of serviceClass?.getInstanceMethods()){
                    ...
                    if(queryDecorator){
                        ...
                        let tenantSelectionParameterName: string | null = null
                        ...
                        for(let i = 0; i < parameters.length; i++){
                            ...
                            } else if(parameter.getType().getAliasSymbol()?.getName() === 'TenantSelection') {
                                tenantSelectionParameterName = parameterName
                        ...
                        if(adminService && !tenantSelectionParameterName){
                            throw new Error(`${entityServiceName}.${methodName} must declare a TenantSelection parameter`)
                        }
                        if(!adminService && tenantSelectionParameterName){
                            throw new Error(`${entityServiceName}.${methodName} cannot declare a TenantSelection parameter, declare the query on the AdminRepository`)
                        }
                        ...
                        // An AdminRepository query also passes its tenant selection
                        // return this.namedQuery('namedQueryTest', parameters, tenantSelection)
                        ...
                        method.setBodyText(writer => {
                            ...
                            const queryArguments = [`'${methodName}'`, argNames.length > 0 ? 'parameters' : '[]']
                            if(tenantSelectionParameterName){
                                queryArguments.push(tenantSelectionParameterName)
                            }
                            if(pageableParameterName){
                                queryArguments.push(pageableParameterName)
                                writer.writeLine(`return this.namedQueryPage(${queryArguments.join(', ')})`)
                            }else{
                                writer.writeLine(`return this.namedQuery(${queryArguments.join(', ')})`)
                            }
                        })
