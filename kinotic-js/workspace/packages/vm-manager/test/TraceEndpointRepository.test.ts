...
function dataDir(): string {
    return mkdtempSync(join(tmpdir(), 'trace-endpoints-'))
}

describe('TraceEndpointRepository', () => {

    it('issues every workload a port and token of its own', () => {
        const repository = new TraceEndpointRepository(dataDir())
    ...
    it('holds nothing for a workload that was never issued one', () => {
        expect(new TraceEndpointRepository(dataDir()).find('wl-1')).toBeNull()
    })

    it('keeps issued endpoints across vm-manager processes', () => {
        const dir = dataDir()
        const issued = new TraceEndpointRepository(dir).issue('wl-1', '172.17.0.1')

        // A guest booted by the previous process still holds this port and token
        expect(new TraceEndpointRepository(dir).find('wl-1')).toEqual(issued)
    })
    ...
    it('refuses to start over a record it cannot read', () => {
        const dir = dataDir()
        writeFileSync(join(dir, 'trace-endpoints.json'), '{not json')

        // Reissuing would hand out ports and tokens the running guests do not hold
        expect(() => new TraceEndpointRepository(dir)).toThrow(/trace endpoints/)
    })

    it('renders the exporter configuration a guest needs, exporting traces alone', () => {
        const environment = TraceEndpointRepository.guestEnvironment('192.168.127.254',
                                                                     { listenAddress: '127.0.0.1', port: 43180, token: 'abc123' })
