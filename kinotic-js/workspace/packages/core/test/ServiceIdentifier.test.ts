  ...
  describe('packages/core', () => {
    describe('ServiceIdentifier construction', () => {
        ...
        it('builds a zone prefixed cri', () => {
            const identifier = new ServiceIdentifier('org.kinotic.management.api.services.iam', 'MemberService', 'os-api')
            identifier.version = '1.0.0'
            expect(identifier.qualifiedName()).toBe('os-api~org.kinotic.management.api.services.iam.MemberService')
            expect(identifier.cri().raw()).toBe('srv://os-api~org.kinotic.management.api.services.iam.MemberService#1.0.0')
