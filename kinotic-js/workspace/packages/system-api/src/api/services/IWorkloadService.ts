...
export interface IWorkloadService extends ICrudServiceProxy<Workload> {
     ...
     * @return a Promise resolving to the number of running workloads
     */
    countRunningForNode(nodeId: string): Promise<number>
...
export class WorkloadServiceProxy extends CrudServiceProxy<Workload> implements IWorkloadService {
    ...
    constructor(kinotic: IKinotic) {
        super(kinotic.serviceProxy(`${SYSTEM_API_ZONE}~org.kinotic.system.api.services.WorkloadService`))
    }
    ...
    public countRunningForNode(nodeId: string): Promise<number> {
        return this.serviceProxy.invoke('countRunningForNode', [nodeId])
    }
