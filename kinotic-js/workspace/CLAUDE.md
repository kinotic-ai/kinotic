...
The `@Publish` decorator takes a `namespace` (required) and optional `name` (defaults to the class name). Related decorators include `@Scope(propertyName)` (for routing to specific service instances) and `@Version` (for semantic versioning).
